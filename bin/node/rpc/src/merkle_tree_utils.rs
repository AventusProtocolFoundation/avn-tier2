use sp_core::{H256, hashing::keccak_256};
use serde::{Serialize, Deserialize};
use jsonrpc_core::{Error as RpcError, ErrorCode, Result};
use crate::{lower_rpc::Error};
...
pub fn generate_tree_root(leaves_data: Vec<Vec<u8>>) -> Result<H256> {
    let mut nodes_hashes: Vec<H256> = leaves_data.into_iter()
    ...
    if root_nodes.len() != 1 {
        return Err(
            RpcError {
                code: ErrorCode::ServerError(Error::ErrorGeneratingRoot.into()),
...
pub fn generate_merkle_path(leaf_data: &Vec<u8>, leaves_data: Vec<Vec<u8>>) -> Result<Vec<H256>> {
    ...
    if leaf_data.is_empty() {
        return Err(
            RpcError {
                code: ErrorCode::ServerError(Error::LeafDataEmpty.into()),
                message: "Error generating merkle path: no leaf data".into(),
                data: None,
            }
        );
    }
    ...
    if leaves_data.is_empty() {
        return Err(
            RpcError {
                code: ErrorCode::ServerError(Error::EmptyLeaves.into()),
                message: "Error generating merkle path: no leaves data".into(),
                data: None,
            }
        );
    }
    ...
    let nodes_hashes = &leaves_data.into_iter()
                                   .map(|data| H256::from_slice(&keccak_256(&data)))
                                   .collect::<Vec<H256>>();
...
fn process_level_for_path(
    ...
    let mut processed_nodes = process_nodes_in_pairs_for_path(node_hash_in_leaf_branch, nodes, merkle_path);

    if processed_nodes.len() > 1 {
        processed_nodes = process_level_for_path(node_hash_in_leaf_branch, &processed_nodes, merkle_path);
    }
...
mod tests {
	...
	fn generate_tree_root_should_return_correct_root_with_leaves() {
        ...
        for number_of_nodes in 1..=mock_trees_roots.len() {
            ...
            assert_eq!(
                ...
                H256(to_array(hex::decode(mock_trees_roots[(number_of_nodes-1) as usize]).unwrap().as_slice()))
            );
        }
    }
    ...
    fn generate_merkle_path_should_return_correct_path() {
        ...
        let mocked_merkle_paths: Vec<(u8, usize, Vec<H256>)> = vec![
            ...
            (2, 0, vec![
                H256(to_array(hex::decode("5fe7f977e71dba2ea1a68e21057beebb9be2ac30c6410aa38d4f3fbe41dcffd2").unwrap().as_slice()))
            ]),
            (2, 1, vec![
                H256(to_array(hex::decode("bc36789e7a1e281436464229828f817d6612f7b477d66591ff96a9e064bcc98a").unwrap().as_slice()))
            ]),
            // Mocked three nodes tree data [0, 1, 2]
            (3, 0, vec![
                H256(to_array(hex::decode("5fe7f977e71dba2ea1a68e21057beebb9be2ac30c6410aa38d4f3fbe41dcffd2").unwrap().as_slice())),
                H256(to_array(hex::decode("f2ee15ea639b73fa3db9b34a245bdfa015c260c598b211bf05a1ecc4b3e3b4f2").unwrap().as_slice()))
            ]),
            (3, 1, vec![
                H256(to_array(hex::decode("bc36789e7a1e281436464229828f817d6612f7b477d66591ff96a9e064bcc98a").unwrap().as_slice())),
                H256(to_array(hex::decode("f2ee15ea639b73fa3db9b34a245bdfa015c260c598b211bf05a1ecc4b3e3b4f2").unwrap().as_slice()))
            ...
            (3, 2, vec![
                H256(to_array(hex::decode("b2521d64679bc4720dabfbae7ce17947a5d373d987d3b0cc1e3042ba2054da4a").unwrap().as_slice()))
            ]),
            // Mocked five nodes tree data [0, 1, 2, 3, 4]
            (5, 0, vec![
                H256(to_array(hex::decode("5fe7f977e71dba2ea1a68e21057beebb9be2ac30c6410aa38d4f3fbe41dcffd2").unwrap().as_slice())),
                H256(to_array(hex::decode("c144ad52449a5832e51e7d4daca4c86a9aafc33d89ef15ff7908956d0edb977d").unwrap().as_slice())),
                H256(to_array(hex::decode("f343681465b9efe82c933c3e8748c70cb8aa06539c361de20f72eac04e766393").unwrap().as_slice()))
            ]),
            (5, 1, vec![
                H256(to_array(hex::decode("bc36789e7a1e281436464229828f817d6612f7b477d66591ff96a9e064bcc98a").unwrap().as_slice())),
                H256(to_array(hex::decode("c144ad52449a5832e51e7d4daca4c86a9aafc33d89ef15ff7908956d0edb977d").unwrap().as_slice())),
            ...
            (5, 2, vec![
                H256(to_array(hex::decode("69c322e3248a5dfc29d73c5b0553b0185a35cd5bb6386747517ef7e53b15e287").unwrap().as_slice())),
                H256(to_array(hex::decode("b2521d64679bc4720dabfbae7ce17947a5d373d987d3b0cc1e3042ba2054da4a").unwrap().as_slice())),
                H256(to_array(hex::decode("f343681465b9efe82c933c3e8748c70cb8aa06539c361de20f72eac04e766393").unwrap().as_slice()))
            ]),
            (5, 3, vec![
            ...
            (5, 4, vec![
                H256(to_array(hex::decode("fecce4ac8ed6fc57f4d880d6af2b443418d564df8f5d52c6782e952564ed79eb").unwrap().as_slice()))
            ])
        ];
