...
fn avn_test_proxy_signed_transfer_succeeds() {
    ...
    ext.execute_with(|| {
        ...
        let call = Box::new(mock::Call::TokenManager(
        ...
        // In order to validate that the proxied call has been dispatched we need any proof that transfer was called.
        // In this case we will check that the Transferred signal was emitted.
        assert!(System::events().iter().any(|a| a.event
            == Event::token_manager(crate::Event::<TestRuntime>::TokenTransferred(
                NON_AVT_TOKEN_ID,
                sender,
...
fn avt_proxy_signed_transfer_succeeds() {
    ...
    ext.execute_with(|| {
        ...
        assert_eq!(
            sender_init_avt_balance - DEFAULT_AMOUNT,
            Balances::free_balance(sender)
        );
        assert_eq!(
            recipient_init_avt_balance + DEFAULT_AMOUNT,
        ...
        assert_eq!(
            NON_ZERO_NONCE + 1,
            <TokenManager as Store>::Nonces::get(sender)
        );
...
fn avn_test_unsupported_proxy_call() {
    let mut ext = ExtBuilder::build_default().as_externality();
    ext.execute_with(|| {
        let call = Box::new(mock::Call::System(frame_system::Call::remark(vec![])));
        assert_noop!(
            TokenManager::proxy(Origin::signed(Default::default()), call),
...
fn avn_test_get_proof_fails_for_invalid_calls() {
    let mut ext = ExtBuilder::build_default().as_externality();
    ext.execute_with(|| {
        let invalid_call = mock::Call::System(frame_system::Call::remark(vec![]));
...
fn avn_test_proxy_gas_costs_paid_correctly() {
    ...
    ext.execute_with(|| {
        ...
        // Prepare the calls
        let inner_call = Box::new(mock::Call::TokenManager(
            super::Call::<TestRuntime>::signed_transfer(
                proof.clone(),
                sender,
                recipient,
        ...
        let outer_call = &mock::Call::TokenManager(token_manager::Call::proxy(inner_call.clone()));
