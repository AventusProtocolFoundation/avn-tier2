...
fn avn_test_lift_to_zero_balance_account_should_succeed() {
    ...
    ext.execute_with(|| {
        ...
        assert_eq!(
            Balances::free_balance(mock_data.receiver_account_id),
            AMOUNT_123_TOKEN
        );

        assert!(System::events().iter().any(|a| a.event
            == Event::token_manager(crate::Event::<TestRuntime>::AVTLifted(
...
fn avn_test_lower_all_avt_token_succeed() {
    ...
    ext.execute_with(|| {
        ...
        assert_eq!(
            Balances::free_balance(from_account_id),
...
// Note: This test prevents the implementation of lower function to use a t2 destination account to receive all the tokens
