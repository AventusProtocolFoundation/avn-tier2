...
mod proxy_signed_rebond {
    ...
    impl RebondContext {
        ...
        fn create_call_for_rebond(&self, sender_nonce: u64) -> Box<<TestRuntime as Config>::Call> {
            ...
            return Box::new(MockCall::ValidatorManager(
                super::super::Call::<TestRuntime>::signed_rebond(
                    proof,
                    self.rebond_value,
                ),
            ));
        ...
        fn create_call_for_rebond_approved_by_relayer(&self, sender_nonce: u64) -> Box<<TestRuntime as Config>::Call> {
    ...
    fn succeeds_with_good_parameters() {
        ...
        ext.execute_with(|| {
            ...
            assert_eq!(Staking::ledger(&controller_account_id).unwrap().active, context.bond_value - context.unbond_value);
            ...
            assert_eq!(Staking::ledger(&controller_account_id).unwrap().active, context.bond_value - context.unbond_value + context.rebond_value);
            ...
            assert_eq!(System::account(stash_account_id).data.misc_frozen, context.bond_value - context.unbond_value + context.rebond_value);
            assert_eq!(System::account(stash_account_id).data.fee_frozen, context.bond_value - context.unbond_value + context.rebond_value);
    ...
    mod fails_when {
        ...
        fn rebond_call_is_unauthorized() {
            ...
            ext.execute_with(|| {
                ...
                assert_noop!(AvnProxy::proxy(context.origin.clone(), rebond_call, None), Error::<TestRuntime>::UnauthorizedSignedRebondTransaction);
            });
        }
    }
}