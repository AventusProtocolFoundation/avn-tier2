...
mod proxy_signed_unbond {
    ...
    impl UnbondContext {
        ...
        fn create_call_for_unbond_approved_by_relayer(&self, sender_nonce: u64) -> Box<<TestRuntime as Config>::Call> {
        ...
        pub fn unbonded_event_emitted(&self) -> bool {
            return System::events().iter().any(|e| {
                e.event == MockEvent::pallet_staking(crate::mock::staking::Event::<TestRuntime>::Unbonded(self.staker.stash.account_id(), self.value))
            });
        }
    }
    ...
    mod fails_when {
        ...
        fn unbond_call_is_unauthorized() {
            ...
            ext.execute_with(|| {
                ...
                assert_noop!(AvnProxy::proxy(context.origin.clone(), unbond_call, None), Error::<TestRuntime>::UnauthorizedSignedUnbondTransaction);
            });
        }
    }
}