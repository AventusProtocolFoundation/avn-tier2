...
mod proxy_signed_withdraw_unbonded {
    ...
    impl WithdrawUnbondedContext {
        fn setup(&self) {
            ...
            // Bond all tokens
            assert_ok!(ValidatorManager::bond(Origin::signed(stash), controller, self.bond_value, RewardDestination::Stash));
        ...
        fn create_call_for_unbond(&self, sender_nonce: u64) -> Box<<TestRuntime as Config>::Call> {
            ...
            return Box::new(MockCall::ValidatorManager(
                super::super::Call::<TestRuntime>::signed_unbond(
                    proof,
                    self.unbonded_value,
                ),
            ));
        ...
        fn create_call_for_withdraw_unbonded(&self, sender_nonce: u64) -> Box<<TestRuntime as Config>::Call> {
        ...
        fn create_call_for_withdraw_unbonded_approved_by_relayer(&self, sender_nonce: u64) -> Box<<TestRuntime as Config>::Call> {
        ...
        fn create_proof_for_signed_withdraw_unbonded(&self, sender_nonce: u64) -> Proof<Signature, AccountId> {
            let controller_account_id = &self.staker.controller.account_id();
        ...
        pub fn withdrawn_event_emitted(&self) -> bool {
            return System::events().iter().any(|e| {
                e.event == MockEvent::pallet_staking(crate::mock::staking::Event::<TestRuntime>::Withdrawn(self.staker.stash.account_id(), self.unbonded_value))
            });
        }
    }
    ...
    fn succeeds_with_good_parameters() {
        ...
        ext.execute_with(|| {
            ...
            assert_eq!(Staking::ledger(&controller_account_id).unwrap().active, context.bond_value - context.unbonded_value);
            ...
            assert_eq!(Staking::ledger(&controller_account_id).unwrap().total, context.bond_value - context.unbonded_value);
            assert_eq!(Staking::ledger(&controller_account_id).unwrap().active, context.bond_value - context.unbonded_value);
            ...
            assert_eq!(System::account(stash_account_id).data.misc_frozen, context.bond_value - context.unbonded_value);
            assert_eq!(System::account(stash_account_id).data.fee_frozen, context.bond_value - context.unbonded_value);

            // Transfer more than unbonded tokens will fail
            assert_noop!(
                Balances::transfer(Origin::signed(*stash_account_id), context.staker.relayer, context.unbonded_value + 1),
                BalancesError::<TestRuntime>::LiquidityRestrictions
            );

            // Transfer equal or less than the unlocked withdrawn unboned amount is successful
            assert_ok!(Balances::transfer(Origin::signed(*stash_account_id), context.staker.relayer, context.unbonded_value));
        });
    }
    ...
    mod fails_when {
        ...
        fn extrinsic_is_unsigned() {
            ...
            ext.execute_with(|| {
                ...
                assert_noop!(AvnProxy::proxy(RawOrigin::None.into(), withdraw_unbonded_call, None), BadOrigin);
            });
        }
        ...
        fn withdraw_unbonded_call_is_unauthorized() {
            ...
            ext.execute_with(|| {
                ...
                let nonce = ValidatorManager::proxy_nonce(controller_account_id);
                let withdraw_unbonded_call = context.create_call_for_withdraw_unbonded_approved_by_relayer(nonce);
                ...
                assert_noop!(AvnProxy::proxy(context.origin.clone(), withdraw_unbonded_call, None), Error::<TestRuntime>::UnauthorizedSignedWithdrawUnbondedTransaction);
            });
        }
    }
}