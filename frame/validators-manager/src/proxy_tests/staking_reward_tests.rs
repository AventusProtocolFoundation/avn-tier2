    ...
    impl PayoutContext {
        fn setup_staker(&self) {
            ...
            assert_ok!(ValidatorManager::bond(
                Origin::signed(stash), controller, self.user_stake_amount, RewardDestination::Stash
            ));
        }
        ...
        fn create_call_for_payout(&self, sender_nonce: u64, era: EraIndex) -> Box<<TestRuntime as Config>::Call> {
            let proof = self.create_proof_for_signed_payout(sender_nonce, era);
            return Box::new(
                MockCall::ValidatorManager(super::super::Call::<TestRuntime>::signed_payout_stakers(proof, era))
            );
        }

        fn create_call_for_payout_approved_by_relayer(&self, sender_nonce: u64, era: EraIndex) -> Box<<TestRuntime as Config>::Call> {
        ...
        fn create_proof_for_signed_payout(&self, sender_nonce: u64, era: EraIndex) -> Proof<Signature, AccountId> {
        ...
        fn create_call_for_nominate(&self, sender_nonce: u64) -> Box<<TestRuntime as Config>::Call> {
            let proof = self.create_proof_for_signed_nominate(sender_nonce);
            return Box::new(
                MockCall::ValidatorManager(super::super::Call::<TestRuntime>::signed_nominate(proof, self.targets.clone()))
            );
        }

        fn create_proof_for_signed_nominate(&self, sender_nonce: u64) -> Proof<Signature, AccountId> {
            let controller_account_id = &self.staker.controller.account_id();
        ...
        pub fn payout_event_emitted(&self, era_index: EraIndex) -> bool {
            return System::events().iter().any(|e| {
                e.event == mock::Event::validators_manager(
                    crate::Event::<TestRuntime>::PayoutCompleted(era_index, self.targets.len() as u32)
                )
            });
        }
    }
    ...
    mod succeeds_when {
        ...
        fn staker_rewards_work() {
            ...
            ext.execute_with(|| {
                ...
                Staking::reward_by_ids(
                    vec![(validator_id_1(), 10), (validator_id_2(), 10), (validator_id_3(), 10), (validator_id_4(), 10), (validator_id_5(), 10)]
                );

                // Set the pot so we can payout
                Balances::make_free_balance_be(&ValidatorManager::account_id(), context.total_payout);
                ...
                let staker_reward =
                    ...
                    get_expected_staker_reward(era_index, validator_id_2(), earning_per_validator) +
                    get_expected_staker_reward(era_index, validator_id_3(), earning_per_validator) +
                    get_expected_staker_reward(era_index, validator_id_4(), earning_per_validator) +
                    get_expected_staker_reward(era_index, validator_id_5(), earning_per_validator);
                ...
                assert_eq_error_rate!(staker_balance + staker_reward, Balances::total_balance(stash_account_id), 2);
            });
        }
        ...
        fn validator_rewards_work_without_commission() {
            ...
            ext.execute_with(|| {
                ...
                let validator1_reward = get_expected_validator_reward(era_index, validator_id_1(), earning_per_validator);
                let validator2_reward = get_expected_validator_reward(era_index, validator_id_2(), earning_per_validator);
                let validator3_reward = get_expected_validator_reward(era_index, validator_id_3(), earning_per_validator);
                let validator4_reward = get_expected_validator_reward(era_index, validator_id_4(), earning_per_validator);
                let validator5_reward = get_expected_validator_reward(era_index, validator_id_5(), earning_per_validator);

                // If there were no rounding issues, locked_era_payout would be 0 but here we allow for pico AVT errors
                assert!(ValidatorManager::locked_era_payout() <= 100000000000);

                // Check the balances
                assert_eq_error_rate!(validator1_balance_before + validator1_reward, Balances::total_balance(&validator_id_1()), 2);
                assert_eq_error_rate!(validator2_balance_before + validator2_reward, Balances::total_balance(&validator_id_2()), 2);
                assert_eq_error_rate!(validator3_balance_before + validator3_reward, Balances::total_balance(&validator_id_3()), 2);
                assert_eq_error_rate!(validator4_balance_before + validator4_reward, Balances::total_balance(&validator_id_4()), 2);
                assert_eq_error_rate!(validator5_balance_before + validator5_reward, Balances::total_balance(&validator_id_5()), 2);
            });
        }
    ...
    mod fails_when {
        ...
        fn payout_stakers_call_is_unauthorized() {
            ...
            ext.execute_with(|| {
                ...
                let era_index = Staking::active_era().unwrap().index;
                let nonce = ValidatorManager::proxy_nonce(context.staker.controller.account_id());
                let payout_call = context.create_call_for_payout_approved_by_relayer(nonce, era_index - 1);

                assert_noop!(AvnProxy::proxy(context.origin.clone(), payout_call, None), Error::<TestRuntime>::UnauthorizedSignedPayoutStakersTransaction);
            });
        }
        ...
        fn era_is_invalid() {
            ...
            ext.execute_with(|| {
                ...
                assert_noop!(AvnProxy::proxy(context.origin.clone(), payout_call, None), StakingError::<TestRuntime>::InvalidEraToReward);
            });
        }
    }
}
...
pub fn get_expected_staker_reward(era_index: EraIndex, validator: AccountId, total_payout: u128) -> u128 {
...
// Phragmen assigns the exposure for each era and its hard to predict so instead we read them from storage
pub fn get_expected_validator_reward(era_index: EraIndex, validator: AccountId, total_payout: u128) -> u128 {
    let validator_prefs = Staking::eras_validator_prefs(&era_index, &validator);
    let exposure = Staking::eras_stakers(era_index, validator);
