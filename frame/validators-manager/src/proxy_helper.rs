...
pub const SIGNED_PAYOUT_STAKERS_CONTEXT: &'static [u8] = b"authorization for signed payout stakers operation";
pub const SIGNED_SET_CONTROLLER_CONTEXT: &'static [u8] = b"authorization for set controller operation";
...
pub fn get_encoded_call_param<T: Config>(call: &<T as Config>::Call) -> Option<(&Proof<T::Signature, T::AccountId>, Vec<u8>)> {
    ...
    match call {
        Call::signed_bond(proof, controller, amount, payee) => {
            ...
            let encoded_data = encode_signed_bond_params::<T>(proof, controller, amount, payee, sender_nonce);
        ...
        Call::signed_set_controller(proof, controller) => {
            let sender_nonce = <ValidatorsManager::<T> as Store>::ProxyNonces::get(&proof.signer);
            let encoded_data = encode_signed_set_controller_params::<T>(proof, controller, sender_nonce);
        ...
        Call::signed_withdraw_unbonded(proof, num_slashing_spans) => {
            let sender_nonce = <ValidatorsManager::<T> as Store>::ProxyNonces::get(&proof.signer);
            let encoded_data = encode_signed_withdraw_unbonded_params::<T>(proof, num_slashing_spans, sender_nonce);
        ...
        Call::signed_bond_extra(proof, max_additional) => {
            let sender_nonce = <ValidatorsManager::<T> as Store>::ProxyNonces::get(&proof.signer);
            let encoded_data = encode_signed_bond_extra_params::<T>(proof, max_additional, sender_nonce);
...
{
    return (SIGNED_BOND_CONTEXT, proof.relayer.clone(), controller, value, payee, sender_nonce).encode();
}
...
pub fn encode_signed_set_controller_params<T: Config>(
    ...
    sender_nonce: u64) -> Vec<u8>
{
    return (SIGNED_SET_CONTROLLER_CONTEXT, proof.relayer.clone(), controller, sender_nonce).encode();
}
...
pub fn encode_signed_withdraw_unbonded_params<T: Config>(
    ...
    sender_nonce: u64) -> Vec<u8>
{
    return (SIGNED_WITHDRAW_UNBONDED_CONTEXT, proof.relayer.clone(), num_slashing_spans, sender_nonce).encode();
}
...
pub fn encode_signed_bond_extra_params<T: Config>(
    ...
    sender_nonce: u64) -> Vec<u8>
{
    return (SIGNED_BOND_EXTRA_CONTEXT, proof.relayer.clone(), max_additional, sender_nonce).encode();
}