...
use sp_runtime::{traits::{BlakeTwo256, IdentityLookup, Verify}, testing::{Header, UintAuthorityId}};
use hex_literal::hex;
...
frame_support::construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic,
    {
        ...
        AvnProxy: avn_proxy::{Module, Call, Storage, Event<T>}
    }
);
...
impl ProvableProxy<Call, Signature, AccountId> for TestAvnProxyConfig {
    fn get_proof(call: &Call) -> Option<Proof<Signature, AccountId>> {
        match call {
            Call::System(system::Call::remark(_msg)) => {
...
impl InnerCallValidator for TestAvnProxyConfig {
    ...
    fn signature_is_valid(call: &Box<Self::Call>) -> bool {
        match **call {
            ...
            Call::NftManager(..) => {
                return pallet_nft_manager::Module::<TestRuntime>::signature_is_valid(call);
            },
            _ => false
        }
    }
...
impl ExtBuilder {
    ...
    pub fn with_balances(mut self) -> Self {
        ...
        let _ = pallet_balances::GenesisConfig::<TestRuntime> {
            balances: vec![(context.signer.account_id(), HUNDRED_AVT), (context.relayer.account_id(), HUNDRED_AVT)],
...
impl ProxyContext {
    ...
    pub fn create_valid_inner_call(&self) -> Box<<TestRuntime as Config>::Call> {
        return Box::new(Call::System(
            SystemCall::remark(vec![]),
        ));
    }

    pub fn create_invalid_inner_call(&self) -> Box<<TestRuntime as Config>::Call> {
        return Box::new(Call::Balances(
            BalancesCall::transfer(Default::default(), Default::default())
        ));
    }

    pub fn create_proxy_call(&self) -> Box<<TestRuntime as Config>::Call> {
        return Box::new(Call::AvnProxy(
            AvnProxyCall::proxy(self.create_valid_inner_call(), None),
        ));
    }
}

pub fn proxy_event_emitted(relayer: AccountId, call_hash: <TestRuntime as system::Config>::Hash) -> bool {
    return System::events().iter().any(|a| {
        a.event == Event::avn_proxy(crate::Event::<TestRuntime>::CallDispatched(relayer, call_hash))
    });
}
...
pub const SIGNED_MINT_SINGLE_NFT_CONTEXT: &'static [u8] = b"authorization for mint single nft operation";

pub fn create_signed_mint_single_nft_call(context: &ProxyContext) -> Box<<TestRuntime as Config>::Call> {
...
pub fn get_signed_mint_single_nft_call(single_nft_data: &SingleNftContext, proof: &Proof<Signature, AccountId>) -> Box<<TestRuntime as Config>::Call> {
    return Box::new(crate::mock::Call::NftManager(
        NftManagerCall::signed_mint_single_nft(
            proof.clone(),
            single_nft_data.unique_external_ref.clone(),
            single_nft_data.royalties.clone(),
            single_nft_data.t1_authority
        ),
    ));
}

pub fn get_mint_single_nft_proxy_proof(context: &ProxyContext, data: &SingleNftContext) -> Proof<Signature, AccountId> {
    let data_to_sign = (
...
pub fn single_nft_minted_events_count() -> usize {
    System::events().into_iter().map(|r| r.event).filter_map(|e| {
        if let Event::pallet_nft_manager(inner) = e {
            Some(inner)
        } else {
            None
        }
