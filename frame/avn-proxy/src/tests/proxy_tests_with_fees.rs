...
pub fn create_default_payment_authorisation(context: &ProxyContext, proxy_proof: Proof<Signature, AccountId>)
    -> PaymentInfo<AccountId, u128, Signature>
{
    return create_payment_authorisation(&context.relayer, &context.signer, proxy_proof, 0_u64);
}

pub fn create_payment_authorisation_with_nonce(context: &ProxyContext, proxy_proof: Proof<Signature, AccountId>, nonce: u64)
    -> PaymentInfo<AccountId, u128, Signature>
{
    let data_to_sign = (PAYMENT_AUTH_CONTEXT, &proxy_proof, &context.relayer.account_id(), &GATEWAY_FEE, nonce);
...
pub fn create_payment_authorisation(
    ...
    nonce: u64) -> PaymentInfo<AccountId, u128, Signature>
{
    let data_to_sign = (PAYMENT_AUTH_CONTEXT, &proxy_proof, &relayer.account_id(), &GATEWAY_FEE, nonce);
    ...
    mod succeeds_when {
        ...
        fn call_is_proxied_with_good_parameters() {
            ...
            ext.execute_with(|| {
                ...
                let payment_authorisation = Some(Box::new(create_default_payment_authorisation(&context, proxy_proof)));
                ...
                assert_ok!(AvnProxy::proxy(Origin::signed(context.relayer.account_id()), inner_call, payment_authorisation));
                ...
                assert_eq!(signer_balance - ONE_AVT, Balances::free_balance(context.signer.account_id()));
                assert_eq!(relayer_balance + ONE_AVT, Balances::free_balance(context.relayer.account_id()));
            })
        }
        ...
        fn signature_is_valid_but_call_is_rejected() {
            ...
            ext.execute_with(|| {
                ...
                // Dispatch fails
                assert_err!(
                    AvnProxy::proxy(Origin::signed(context.relayer.account_id()), inner_call, payment_authorisation),
        ...
        fn payer_and_signer_are_different() {
            ...
            ext.execute_with(|| {
                ...
                let signer_balance = Balances::free_balance(context.signer.account_id());
                let relayer_balance = Balances::free_balance(context.relayer.account_id());
                let new_payer_balance = Balances::free_balance(new_payment_proof_signer.account_id());
                ...
                assert_eq!(new_payer_balance - ONE_AVT, Balances::free_balance(new_payment_proof_signer.account_id()));
    ...
    mod fails_when {
        ...
        fn wrong_fee_is_signed() {
            ...
            ext.execute_with(|| {
                ...
                let mut payment_authorisation = create_default_payment_authorisation(&context, proxy_proof);
        ...
        fn wrong_nonce_is_signed() {
            ...
            ext.execute_with(|| {
                ...
                let payment_authorisation = create_payment_authorisation_with_nonce(&context, proxy_proof, bad_nonce);
        ...
        fn invalid_inner_call_proof_given() {
            ...
            ext.execute_with(|| {
                ...
                let inner_call = get_signed_mint_single_nft_call(&single_nft_data, &proxy_proof);
                let payment_authorisation = create_default_payment_authorisation(&context, proxy_proof);
        ...
        fn inner_call_proof_does_not_match_fee_proof() {
            ...
            ext.execute_with(|| {
                let context: ProxyContext = Default::default();
                let single_nft_data: SingleNftContext = Default::default();
                let original_proxy_proof = get_mint_single_nft_proxy_proof(&context, &single_nft_data);
                let original_payment_authorisation = create_default_payment_authorisation(&context, original_proxy_proof.clone());
                let original_inner_call = get_signed_mint_single_nft_call(&single_nft_data, &original_proxy_proof);

                // Generate a new valid proof with different data
                let mut new_single_nft_data: SingleNftContext = single_nft_data.clone();
                new_single_nft_data.unique_external_ref = String::from("New unique ref").into_bytes();
                let new_proxy_proof = get_mint_single_nft_proxy_proof(&context, &new_single_nft_data);
                let new_payment_authorisation = create_default_payment_authorisation(&context, new_proxy_proof.clone());
                let new_inner_call = get_signed_mint_single_nft_call(&new_single_nft_data, &new_proxy_proof);
                ...
                assert_ok!(
                    AvnProxy::proxy(Origin::signed(context.relayer.account_id()),
                    original_inner_call,
