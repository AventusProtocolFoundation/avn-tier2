...
pub fn create_and_report_summary_offence<T: crate::Config>(
    ...
    if offenders.len() > 0 {
        ...
        if !T::ReportSummaryOffence::is_known_offence(
            ...
            if let Err(e) = T::ReportSummaryOffence::report_offence(reporters, invalid_event_offence) {
                debug::native::info!(
            ...
            <crate::Module<T>>::deposit_event(Event::<T>::SummaryOffenceReported(offence_type, offenders));
        }
    }
}
