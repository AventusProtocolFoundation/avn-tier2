...
use sp_core::{ecdsa, H256, offchain::testing::PoolState};
...
fn record_summary_calculation_is_called(
    ...
    Summary::process_summary_if_required(
...
fn get_unsigned_record_summary_calculation_call_from_chain(pool_state: &Arc<RwLock<PoolState>>) -> Call<TestRuntime> {
...
fn expected_unsigned_record_summary_calculation_call(context: &Context) -> Call<TestRuntime> {
    ...
    return Call::record_summary_calculation(
        context.last_block_in_range,
        context.root_hash_h256,
        context.root_id.ingress_counter,
    ...
    mod calls_record_summary_calculation_successfully {
        ...
        fn when_primary_validator_processes_current_block() {
            ...
            ext.execute_with(|| {
                ...
                let submitted_unsigned_transaction_call = get_unsigned_record_summary_calculation_call_from_chain(&pool_state);
    ...
    mod stops_to_call_record_summary_calculation_when {
        ...
        fn slot_expires_and_process_summary_is_not_successful() {
            ...
            ext.execute_with(|| {
                ...
                // Advance to block #12
                // By providing a successful compute root hash response, the validator should not be able to reach it
                // and call the transaction as the slot has expired
                let current_block_number = advance_block_numbers(1);
                mock_response_of_get_roothash(
                    &mut offchain_state.write(),
                    context.url_param.clone(),
                ...
                // Retreat to the previous block #11 and call it again to show a successful unsigned transaction is created
                // This proves the failure in block #12 is not caused by any other reasons except the block number is
...
mod process_summary {
    ...
    fn succeeds_then_a_record_summary_calculation_transaction_is_created() {
        ...
        ext.execute_with(|| {
            ...
            assert_eq!(
                tx.call,
                mock::Call::Summary(Call::record_summary_calculation(
                    context.last_block_in_range,
                    context.root_hash_h256,
                    expected_ingress_counter,
                    context.validator.clone(),
    ...
    mod fails {
            ...
            fn invalid_utf_8_bytes() {
                ...
                ext.execute_with(|| {
                    ...
                    let invalid_utf8_bytes = vec![
                        0, 159, 146, 150, 0, 159, 146, 150, 0, 159, 146, 150, 0, 159, 146, 150, 0, 159,
                        146, 150, 0, 159, 146, 150, 0, 159, 146, 150, 0, 159, 146, 150, 0, 159, 146,
                        150, 0, 159, 146, 150, 0, 159, 146, 150, 0, 159, 146, 150, 0, 159, 146, 150, 0,
                        159, 146, 150, 0, 159, 146, 150, 0, 159, 146, 150,
                    ];
        ...
        fn when_record_summary_has_error_in_signing() {
            ...
            ext.execute_with(|| {
                ...
                // TODO [TYPE: test][PRI: medium][JIRA: 321]: Mock a validator to cause signing error
        ...
        fn when_record_summary_has_error_in_submitting_transaction() {
            ...
            ext.execute_with(|| {
                ...
                // TODO [TYPE: test][PRI: medium][JIRA: 321]: Mock a submit_unsigned_transaction with error
                assert_noop!(
                    Summary::process_summary(context.last_block_in_range, &context.validator),
...
pub mod record_summary_calculation {
    ...
    mod succeeds_implies_that {
        ...
        fn root_data_is_correctly_added() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(
                    root,
                    RootData::new(context.root_hash_h256, context.validator.account_id.clone(), root.tx_id)
                );
            });
        }
        ...
        fn when_next_block_to_process_overflows() {
            ...
            ext.execute_with(|| {
                ...
                let last_block_in_range =
                    next_block_to_process - Summary::schedule_period() + 1;
                System::set_block_number(context.current_block_number);
    ...
    mod succeeds_when {
        ...
        fn a_rejected_summary_is_recreated() {
            ...
            ext.execute_with(|| {
                ...
                // Validate the same unsigned record_summary_calculation extrinsic with a new signature and increased ingress counter
    ...
    fn get_context_with_new_ingress_counter(context: &Context, ingress_counter: IngressCounter) -> Context {
        let mut new_context = context.clone();
        new_context.root_id.ingress_counter = ingress_counter;
        new_context.record_summary_calculation_signature = get_signature_for_record_summary_calculation(
            context.validator.clone(),
            UPDATE_BLOCK_NUMBER_CONTEXT,
            context.root_hash_h256,
            ingress_counter,
...
// TODO [TYPE: tests][PRI: low]: Increase the scheduled period and test at least 3 summaries have been processed successfully here.
mod if_process_summary_is_called_a_second_time {
    use super::*;

    const SECOND_ROOT_HASH_HEX_STRING: &'static [u8; 64] = b"09ec14e7d5fe581fe4ef4631112038297dcdecae163e8724c281ece8ad94c8c3";
    const SECOND_ROOT_HASH_BYTES: [u8; 32] = [
        9, 236, 20, 231, 213, 254, 88, 31, 228, 239, 70, 49, 17, 32, 56, 41, 125, 205, 236, 174, 22,
    ...
    fn setup_block_numbers_and_slots(context: &Context) {
        ...
        let block_number_for_next_slot: BlockNumber = current_block_number + Summary::schedule_period();
    ...
    fn setup_second_process_summary_context(current_block_number: BlockNumber) -> Context {
        ...
        let tx_id = TestRuntime::reserve_transaction_id(
            &EthTransactionType::PublishRoot(PublishRootData::new(*root_hash_h256.as_fixed_bytes()))
        ).unwrap();
        let data_to_sign = Summary::convert_data_to_eth_compatible_encoding(
            &RootData::<u64>::new(
        ...
        Context {
            ...
            root_hash_h256: root_hash_h256,
            root_hash_vec: root_hash_vec,
    ...
    fn approve_summary(context: &Context) {
        ...
        assert_eq!(
            Summary::last_summary_slot(),
            Summary::current_slot()
        );
    ...
    fn reject_summary(context: &Context) {
        ...
        assert_eq!(
            Summary::last_summary_slot(),
            previous_summary_slot_before_voting
        );
    ...
    mod given_the_first_summary_was_approved {
        ...
        fn then_second_call_processes_second_range(){
            ...
            ext.execute_with(|| {
                ...
                let last_block_in_range_before_first_time_process_summary = Summary::get_target_block().expect("Valid block number");
                let url_param_for_first_summary_process = first_process_summary_context.url_param.clone();

                // Mock a successful compute root hash response for the first time process summary
                let fake_successful_response_1 = first_process_summary_context.root_hash_vec.clone();
                mock_response_of_get_roothash(
                    &mut offchain_state.write(),
                ...
                // The first time process summary for root [from:3;to:4] is created successfully at block#12
                assert!(record_summary_calculation_is_called(
                ...
                let submitted_unsigned_transaction_call = get_unsigned_record_summary_calculation_call_from_chain(&pool_state);
                let expected_call = expected_unsigned_record_summary_calculation_call(&first_process_summary_context);
                assert_eq!(submitted_unsigned_transaction_call, expected_call);
                ...
                let second_process_summary_context = setup_second_process_summary_context(current_block_number);

                // NextBlockToProcess is increased by 1 as the first processed summary is approved
                assert_eq!(second_process_summary_context.last_block_in_range, last_block_in_range_before_first_time_process_summary + Summary::schedule_period());
                // Url Param for the target root is updated from [from:3;to:4] to [from:5;to:6] as the first processed summary is approved
                assert!(second_process_summary_context.url_param != url_param_for_first_summary_process);

                // Mock successful compute root hash response for the second summary process
                let fake_successful_response_2 = second_process_summary_context.root_hash_vec.clone();
                mock_response_of_get_roothash(
                    &mut offchain_state.write(),
                ...
                // Process summary for the new root [from:5;to:6], that is different from the first time is successfully created
                assert!(record_summary_calculation_is_called(
                    current_block_number,
                ...
                let submitted_unsigned_transaction_call = get_unsigned_record_summary_calculation_call_from_chain(&pool_state);
                let expected_call = expected_unsigned_record_summary_calculation_call(&second_process_summary_context);
                assert_eq!(submitted_unsigned_transaction_call, expected_call);
    ...
    mod given_the_first_summary_was_rejected {
        ...
        fn then_second_call_processes_first_range() {
            ...
            ext.execute_with(|| {
                ...
                // NextBlockToProcess is not increased as the processed summary for the first time is rejected
                assert_eq!(second_process_summary_context.last_block_in_range, last_block_in_range_before_first_time_process_summary);
                // Url Param for target root is not updated from [from:3;to:4] to [from:5;to:6] as the processed summary for the first time is rejected
                assert_eq!(second_process_summary_context.url_param, url_param_for_first_summary_process);

                // Mock successful compute root hash response to process summary for the second time [from:3;to:4]
                // This is a new root hash value that is different from the first time
                let fake_successful_response_2 = second_process_summary_context.root_hash_vec.clone();
                mock_response_of_get_roothash(
                    &mut offchain_state.write(),
                    second_process_summary_context.url_param.clone(),
    ...
    mod given_the_first_summary_is_pending {
        ...
        fn then_second_call_has_no_effect() {
            ...
            ext.execute_with(|| {
                ...
                // NextBlockToProcess is not increased as the processed summary for the first time is still pending for approval
                assert_eq!(second_process_summary_context.last_block_in_range, last_block_in_range_before_first_time_process_summary);
