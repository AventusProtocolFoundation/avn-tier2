...
use crate::tests::assert_record_summary_calculation_is_ok;
use crate::tests_vote::{
    vote_to_approve_root,
    vote_to_reject_root,
    setup_approved_root
};
use crate::avn::vote::{APPROVE_VOTE_IS_NOT_VALID, REJECT_VOTE_IS_NOT_VALID, VOTE_SESSION_IS_NOT_VALID};
...
pub mod input_is_record_summary_calculation {
    ...
    fn succeeds() {
        ...
        ext.execute_with(|| {
            ...
            let transaction_call = Call::record_summary_calculation(
    ...
    mod fails_when {
        ...
        fn test_record_summary_calculation_call_fails(call: &Call<TestRuntime>) {
            let (mut ext, _pool_state, _offchain_state) = ExtBuilder::build_default()
                .with_validators()
                .for_offchain_worker()
                .as_externality_with_state();
        ...
        fn validator_is_not_primary() {
            ...
            ext.execute_with(|| {
                ...
                let record_summary_calculation_call = Call::record_summary_calculation(
                    context.last_block_in_range,
                    context.root_hash_h256,
                    context.root_id.ingress_counter,
                    second_validator,
                    second_validator_signed_signature
                );
        ...
        fn from_non_validator() {
            ...
            ext.execute_with(|| {
                ...
                let non_validator_signed_signature =
                    get_signature_for_record_summary_calculation(
                        non_validator.clone(),
                        UPDATE_BLOCK_NUMBER_CONTEXT,
                        context.root_hash_h256,
                ...
                let record_summary_calculation_call = Call::record_summary_calculation(
                    ...
                    context.root_id.ingress_counter,
                    non_validator,
                    non_validator_signed_signature
                );
        ...
        mod signature_has_wrong {
            ...
            fn get_call(context: &Context, signature: TestSignature) -> Call<TestRuntime> {
            ...
            fn signature_context() {
                ...
                ext.execute_with(|| {
                    ...
                    let record_summary_calculation_call = get_call(&context, wrong_context_signature);
        ...
        fn signature_is_reused() {
            ...
            ext.execute_with(|| {
                ...
                // Reuse the signature to validate the same unsigned record_summary_calculation extrinsic
                assert_validate_unsigned_record_summary_calculation_is_successful(&context);

                // Reuse the signature to validate unsigned record_summary_calculation extrinsic with a different ingress counter
                let transaction_call = Call::record_summary_calculation(
                    context.last_block_in_range,
                    context.root_hash_h256.clone(),
                ...
                // Reuse the signature to execute unsigned record_summary_calculation extrinsic with the same ingress counter
                assert_noop!(
    ...
    mod fails_when {
        ...
        mod signature_has_wrong {
            ...
            fn signer() {
                ...
                ext.execute_with(|| {
                    let context = setup_context();
                    let second_validator = get_validator(SECOND_VALIDATOR_INDEX);
                    let signature_signed_by_different_validator = get_signature_for_end_voting_period(
                        second_validator,
                        END_VOTING_PERIOD_CONTEXT,
                        &context.root_id
                    );

                    let end_voting_period_call = Call::end_voting_period(
                        context.root_id,
                        context.validator.clone(),
                        signature_signed_by_different_validator,
                    );
            ...
            fn root_id() {
                ...
                ext.execute_with(|| {
                    ...
                    let wrong_root_id = RootId::new(
                        RootRange::new(context.root_id.range.from_block + 1, context.root_id.range.to_block + 1),
                        context.root_id.ingress_counter
                    );
        ...
        fn signature_is_reused() {
            ...
            ext.execute_with(|| {
                ...
                // Reuse the signature to validate the same unsigned end_voting_period extrinsic again
                let end_voting_period_call = Call::end_voting_period(
                    context.root_id,
                    context.validator.clone(),
                    signature.clone(),
                ...
                assert_noop!(
                    <Summary as ValidateUnsigned>::validate_unsigned(TransactionSource::Local, &end_voting_period_call),
                    InvalidTransaction::Custom(VOTE_SESSION_IS_NOT_VALID)
                );

                // Reuse the signature to validate unsigned end_voting_period extrinsic with a different ingress counter
                let root_id = RootId::new(
                    context.root_id.range,
                    context.root_id.ingress_counter + 1
                );
                let end_voting_period_call = Call::end_voting_period(
                    root_id,
                ...
                // Reuse the signature to execute the same unsigned end_voting_period extrinsic again
                assert_noop!(
...
fn get_cast_vote_call(context: &Context, is_approve_root: bool) -> Call<TestRuntime> {
    if is_approve_root {
...
fn test_signature_is_wrong(context: &Context, call: &Call<TestRuntime>) {
    let (mut ext, _pool_state, _offchain_state) = ExtBuilder::build_default()
        .with_validators()
        .for_offchain_worker()
        .as_externality_with_state();
    ...
    mod fails_when {
        ...
        mod vote_is_invalid {
            ...
            fn for_non_validator() {
                ...
                ext.execute_with(|| {
                    ...
                    let approve_root_call = Call::approve_root(
                        context.root_id,
                        non_validator,
                        context.approval_signature.clone(),
                        signature,
        ...
        mod signature_has_wrong {
            ...
            fn signer() {
                ...
                ext.execute_with(|| {
                    let context = setup_context();
                    let second_validator = get_validator(SECOND_VALIDATOR_INDEX);
                    let signature_signed_by_different_validator = get_signature_for_approve_cast_vote(
                        &second_validator,
                        CAST_VOTE_CONTEXT,
                        &context.root_id,
                        &context.sign_url_param,
            ...
            fn approve_root() {
                ...
                ext.execute_with(|| {
                    ...
                    let approve_root_call = Call::approve_root(
                        ...
                        context.validator.clone(),
                        context.approval_signature.clone(),
                        signature_with_wrong_approve_root,
                    );
        ...
        fn signature_is_reused() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(
                    <Summary as ValidateUnsigned>::validate_unsigned(TransactionSource::Local, &approve_root_call),
                ...
                // Reuse the signature to validate unsigned approve_root extrinsic with a different ingress counter
                let root_id = RootId::new(
                    context.root_id.range,
                    context.root_id.ingress_counter + 1
                );
    ...
    mod fails_when {
        ...
        mod vote_is_invalid {
            ...
            fn for_non_validator() {
                ...
                ext.execute_with(|| {
                    ...
                    let reject_root_call = Call::reject_root(
                        context.root_id,
                        non_validator,
                        signature
                    );
        ...
        mod signature_has_wrong {
            ...
            fn signer() {
                ...
                ext.execute_with(|| {
                    let context = setup_context();
                    let second_validator = get_validator(SECOND_VALIDATOR_INDEX);
                    let signature_signed_by_different_validator = get_signature_for_reject_cast_vote(
                        &second_validator,
                        CAST_VOTE_CONTEXT,
                        &context.root_id,
                    );
            ...
            fn reject_root() {
                ...
                ext.execute_with(|| {
                    ...
                    let reject_root_call = Call::reject_root(
                        context.root_id,
                        context.validator.clone(),
                        signature_with_wrong_reject_root,
                    );
        ...
        fn signature_is_reused() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(
                    <Summary as ValidateUnsigned>::validate_unsigned(TransactionSource::Local, &reject_root_call),
                ...
                // Reuse the signature to validate unsigned reject_root extrinsic with a different ingress counter
                let root_id = RootId::new(
                    context.root_id.range,
                    context.root_id.ingress_counter + 1
                );
