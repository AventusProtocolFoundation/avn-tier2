...
use sp_runtime::{testing::{UintAuthorityId, TestSignature}, traits::BadOrigin};
...
mod advance_slot {
    ...
    pub mod _if_required {
        ...
        pub fn call_advance_slot_if_required (
        ...
        fn expected_transaction_was_called(
            validator: &MockValidator,
            pool_state: &Arc<RwLock<PoolState>>) -> bool
        {
        ...
        fn take_transaction_from_pool(pool_state: &Arc<RwLock<PoolState>>) -> Call<TestRuntime> {
        ...
        fn expected_advance_slot_transaction(validator: &MockValidator) -> Call<TestRuntime> {
            let signature = validator.key
                .sign(&(ADVANCE_SLOT_CONTEXT, Summary::current_slot()).encode())
                .expect("Signature is signed");

            return Call::advance_slot(validator.clone(), signature);
        }
        ...
        mod does_not_call_advance_slot_if {
            ...
            fn called_by_non_slot_validator() {
                ...
                ext.execute_with(|| {
                    ...
                    call_advance_slot_if_required(context.block_number_for_next_slot, &other_validator);
    ...
    mod can_successfully_be_called {
        ...
        fn by_other_validator_after_grace_period() {
            ...
            ext.execute_with(|| {
                ...
                let after_grace_period = context.block_number_for_next_slot + context.grace_period + 1;
    ...
    mod post_conditions {
        ...
        fn computes_new_validator() {
            ...
            ext.execute_with(|| {
                ...
                let old_validator = Summary::slot_validator();
                assert_ok!(call_advance_slot(&validator, signature));
        ...
        fn does_not_report_a_slot_not_advanced_offence() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(false, Summary::reported_offence_of_type(SummaryOffenceType::SlotNotAdvanced));
                assert_eq!(false, Summary::emitted_event_for_offence_of_type(SummaryOffenceType::SlotNotAdvanced));
            });
        }
    }
    ...
    mod auxiliary_logic {
        ...
        fn successive_slots_have_consecutive_validators() {
            ...
            ext.execute_with(|| {
                ...
                let validator = get_validator(Summary::slot_validator());
                let signature = create_signature(Summary::current_slot(), &validator);
    ...
    mod advance_slot {
        ...
        fn includes_all_relevant_fields() {
            ...
            ext.execute_with(||{
                ...
                advance_slot::_if_required::call_advance_slot_if_required(context.block_number_for_next_slot, &validator);
        ...
        mod is_rejected_when {
            ...
            fn submitted_by_non_validator() {
                ...
                ext.execute_with(|| {
                    ...
                    match tx.call {
                        mock::Call::Summary(crate::Call::advance_slot(_validator, signature)) => {
                            let data = &(ADVANCE_SLOT_CONTEXT, context.slot_number);
...
mod cases_for_no_summary_created_offences {
    ...
    mod when_slot_is_advanced_and {
        ...
        pub fn setup_approved_root(context: &LocalContext, root_context: RootContext) {
            ...
            assert_eq!(false, vec![validator.clone(), context.other_validator.clone()].contains(&third_validator));
            ...
            assert_ok!(Summary::end_voting_period(RawOrigin::None.into(), root_id, validator.clone(), record_summary_signature));
        }
        ...
        mod summaries_were_not_created_for_several_slots {
            ...
            fn several_events_are_emitted() {
                ...
                ext.execute_with(|| {
                    ...
                    let offence_event = mock::Event::summary(
                        Event::<TestRuntime>::SummaryNotPublishedOffence(
                            validator.account_id,
                            context.slot_number,
                            last_summary_slot,
                            context.block_number_for_next_slot,
                    ...
                    let offenders = vec![Summary::create_mock_identification_tuple(validator.account_id)];

                    let offence_reported_event = mock::Event::summary(
                        Event::<TestRuntime>::SummaryOffenceReported(
                    ...
                    // Show that we have accounted for all events, and thus not created an offence for each missing summary
            ...
            fn an_offence_is_reported_by_someone_else_after_grace_period() {
                ...
                ext.execute_with(|| {
                    ...
                    System::set_block_number(context.current_block + context.grace_period + arbitrary_margin);
        ...
        mod one_summary_is_created {
            ...
            fn no_event_is_emitted() {
                ...
                ext.execute_with(|| {
                    ...
                    let root_context = RootContext {
                        root_id: RootId::new(
                            RootRange::new(Summary::get_next_block_to_process(), context.summary_last_block_in_range),
                    ...
                    let signature = create_signature(Summary::current_slot(), &context.slot_validator);
                    assert_ok!(call_advance_slot(&context.slot_validator, signature));

                    let new_slot_number = Summary::current_slot();
                    assert_eq!(new_slot_number, old_slot_number + 1);

                    // show that no events (with any parameters) related to an offence has been emitted
            ...
            fn no_offence_is_reported() {
                ...
                ext.execute_with(|| {
                    ...
                    assert_eq!(false, Summary::reported_offence_of_type(SummaryOffenceType::NoSummaryCreated));
                });
            }
        }
        mod multiple_summaries_are_created {
            ...
            fn no_event_is_emitted() {
                ...
                ext.execute_with(|| {
                    ...
                    assert_eq!(Summary::get_next_block_to_process(), context.summary_last_block_in_range + 1);
                    ...
                    let summary_last_block_in_range = new_from_block + Summary::schedule_period() - 1;
                    let root_context_2 = RootContext {
                        root_id: RootId::new(
                            RootRange::new(Summary::get_next_block_to_process(), summary_last_block_in_range),
                            DEFAULT_INGRESS_COUNTER
                        ),
                        root_hash: H256::from(ROOT_HASH_BYTES),
                        tx_id: 2,
                    ...
                    assert_eq!(Summary::last_summary_slot(), context.slot_number);
                    assert_eq!(Summary::get_next_block_to_process(), summary_last_block_in_range + 1);

                    // Advance block to the end of the slot (set to block #14) so we can advance the slot
                    advance_block_numbers(1);
...
fn event_is_a_not_published_offence(e: &mock::Event) -> bool {
    if let mock::Event::summary(Event::<TestRuntime>::SummaryNotPublishedOffence(_, _, _, _)) = &e {
        return true;
    } else {
        return false;
    }
...
fn add_offence_event_emitted() -> bool {
    return System::events().iter().any(|e| {
        event_is_a_not_published_offence(&e.event)
    });
}