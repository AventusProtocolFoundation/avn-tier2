...
use pallet_avn::{self as avn, vote::VotingSessionData, EthereumPublicKeyChecker,
    testing::{U64To32BytesConverter}
};
...
impl Summary {
    ...
    pub fn insert_root_hash(root_id: &RootId<BlockNumber>, root_hash: H256, account_id: AccountId, tx_id: TransactionId) {
        <<Summary as Store>::Roots>::insert(root_id.range, root_id.ingress_counter, RootData::new(root_hash, account_id, Some(tx_id)));
    }

    pub fn set_schedule_and_voting_periods(schedule_period: BlockNumber, voting_period: BlockNumber) {
    ...
    pub fn set_root_as_validated(root_id: &RootId<BlockNumber>) {
        <<Summary as Store>::Roots>::mutate(root_id.range, root_id.ingress_counter, |root| root.is_validated = true);
    }
    ...
    pub fn get_vote_for_root(root_id: &RootId<BlockNumber>) -> VotingSessionData<AccountId, BlockNumber> {
    ...
    pub fn event_matches_offence_type(event: &Event, this_type: SummaryOffenceType) -> bool {
        return matches!(event,
            mock::Event::summary(
                crate::Event::<TestRuntime>::SummaryOffenceReported(offence_type, _)
            )
    ...
    pub fn reported_offence_of_type(
        offence_type: SummaryOffenceType
    ) -> bool {
        let offences = Self::get_offence_record();

        return offences.iter().any(|o|
            Self::offence_is_of_type(
                o,
                offence_type.clone()
            ));
    }
...
frame_support::construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic,
    {
        ...
        Summary: summary::{Module, Call, Storage, Event<T>, Config<T>}
    }
);
...
thread_local! {
    ...
    static MOCK_RECOVERED_ACCOUNT_ID: RefCell<u64> = RefCell::new(FIRST_VALIDATOR_INDEX);
}
...
impl CandidateTransactionSubmitter<AccountId> for TestRuntime {
    fn submit_candidate_transaction_to_tier1(
        ...
        if candidate_type != EthTransactionType::PublishRoot(PublishRootData::new(ROOT_HASH_CAUSES_SUBMISSION_TO_T1_ERROR)) {
            return Ok(());
        }
...
impl pallet_session::historical::SessionManager<ValidatorId, FullIdentification> for TestSessionManager {
...
pub fn set_mock_recovered_account_id(account_id: u64) {
    MOCK_RECOVERED_ACCOUNT_ID.with(|acc_id| { *acc_id.borrow_mut() = account_id; });
}
...
pub fn setup_context() -> Context {
    ...
    Context {
        ...
        last_block_in_range: last_block_in_range,
...
pub fn get_signature_for_reject_cast_vote(
    ...
    validator.key.sign(&(context, root_id.encode(), REJECT_ROOT).encode()).expect("Signature is signed")
}
...
pub fn advance_block_numbers(number_of_blocks: u64) -> BlockNumber {
    ...
    let new_block_number = safe_add_block_numbers(now, number_of_blocks)
        .expect("Advanced block number is valid");
...
pub fn retreat_block_numbers(number_of_blocks: u64) -> BlockNumber {
    let now = System::block_number().max(1);
    let new_block_number = safe_sub_block_numbers(now, number_of_blocks)
        .expect("Retreated block number is valid");
    System::set_block_number(new_block_number);
    return new_block_number;
}
