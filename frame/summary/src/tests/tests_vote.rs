...
pub fn vote_to_approve_root(validator: &Validator<UintAuthorityId, u64>, context: &Context) -> bool {
    ...
    mod succeeds {
        ...
        fn when_two_validators_vote_differently() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(Result::Ok(()), Summary::approve_root(
                    RawOrigin::None.into(),
                    context.root_id,
    ...
    mod does_not_send_transactions {
        ...
        fn when_setting_lock_with_expiry_has_error() {
            ...
            ext.execute_with(|| {
                ...
                // TODO [TYPE: test][PRI: medium][JIRA: 321]: mock of set_lock_with_expiry returns error
...
mod end_voting_period {
    ...
    mod fails {
        ...
        mod when_end_voting {
            ...
            // More tests after the TODO when we didn't get enough votes to approve this root has been implemented
        }
    }
    ...
    mod offence_logic {
        ...
        mod when_root_is_approved {
            ...
            fn setup_approved_root(context: &Context) -> (
            ...
            fn reports_offence_for_nay_voters_only() {
                ...
                ext.execute_with(|| {
                    ...
                    assert_eq!(false, Summary::reported_offence_of_type(SummaryOffenceType::CreatedInvalidRoot));
                    assert_eq!(false, Summary::reported_offence_of_type(SummaryOffenceType::ApprovedInvalidRoot));
                });
            }
        }
        ...
        mod when_root_is_rejected {
            ...
            fn setup_rejected_root(context: &Context) -> (
                    Vec<ValidatorId>, // ayes
                    Vec<ValidatorId>  // nays
                ) {
                let indices = validator_indices();
            ...
            fn reports_offence_for_aye_voters_only() {
                ...
                ext.execute_with(|| {
                    ...
                    assert_eq!(false, Summary::reported_offence_of_type(SummaryOffenceType::RejectedValidRoot));
                });
            }
        }
        ...
        mod when_vote_has_no_outcome {
            ...
            fn setup_root_without_outcome(context: &Context) -> (
                    Vec<ValidatorId>, // ayes
                    Vec<ValidatorId>  // nays
                ) {
                let indices = validator_indices();
