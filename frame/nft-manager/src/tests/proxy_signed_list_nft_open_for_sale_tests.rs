...
impl Context {
    ...
    fn open_for_sale_event_emitted(&self) -> bool {
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::NftOpenForSale(
                    self.nft_id,
                    self.market,
                ))
        });
    ...
    mod fails_when {
        ...
        fn nft_already_listed_for_sale() {
            ...
            ext.execute_with(|| {
                let context = Context::default();
                context.setup();
                <NftOpenForSale>::insert(context.nft_id, NftSaleType::Ethereum);
                let call = context.create_signed_list_nft_open_for_sale_call();
        ...
        fn nft_is_locked() {
            ...
            ext.execute_with(|| {
                let context = Context::default();
                context.setup();
                <Nfts<TestRuntime>>::mutate(context.nft_id, |nft| {
                    nft.is_locked = true;
                });
                let call = context.create_signed_list_nft_open_for_sale_call();
        ...
        fn call_is_invalid() {
            ...
            ext.execute_with(|| {
                let context = Context::default();
                context.setup();
                let invalid_call = Box::new(MockCall::System(SystemCall::remark(vec![])));
        ...
        fn mismatched_proof_nft_nonce() {
            ...
            ext.execute_with(|| {
                ...
                for bad_nonce in bad_nonces.iter() {
                    ...
                    assert_eq!(original_nonce, NftManager::nfts(context.nft_id).nonce);
