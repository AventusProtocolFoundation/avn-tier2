...
impl Context {
    ...
    fn mint_single_nft_event_emitted(&self) -> bool {
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::SingleNftMinted(
                    self.nft_id,
                    self.nft_owner_account,
                    self.t1_authority,
                ))
        ...
        fn nft_id_is_created() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(
                    ...
                    <Nfts<TestRuntime>>::get(&context.nft_id)
                );
            });
        }
    ...
    mod fails_when {
        ...
        fn unique_external_ref_is_already_in_use() {
            ...
            ext.execute_with(|| {
                let context = Context::default();
                context.setup();
                <UsedExternalReferences>::insert(&context.unique_external_ref, true);
                let call = context.create_signed_mint_single_nft_call();
