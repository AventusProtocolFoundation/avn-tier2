...
impl Context {
    ...
    fn create_signed_transfer_fiat_nft_call(&self) -> Box<<TestRuntime as Config>::Call> {
    ...
    fn signed_transfer_fiat_nft_event_emitted(&self) -> bool {
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::FiatNftTransfer(
                    self.nft_id,
                    self.nft_owner_account,
                    self.new_nft_owner_account,
                    NftSaleType::Fiat,
                    self.op_id,
                ))
        });
