...
impl Context {
    ...
    fn create_signed_cancel_list_fiat_nft_call(&self) -> Box<<TestRuntime as Config>::Call> {
        ...
        return Box::new(MockCall::NftManager(
            super::Call::<TestRuntime>::signed_cancel_list_fiat_nft(proof, self.nft_id),
        ));
    }
    ...
    fn signed_cancel_list_fiat_nft_event_emitted(&self) -> bool {
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::CancelSingleFiatNftListing(
                    self.nft_id,
                    NftSaleType::Fiat,
                    self.op_id,
                ))
    ...
    fn call_dispatched_event_emitted(&self, call: &Box<<TestRuntime as Config>::Call>) -> bool {
        ...
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::CallDispatched(
                    relayer.account_id(),
                    Hashing::hash_of(call),
                ))
        });
    }
}
    ...
    mod succeeds_implies_that {
        ...
        fn nft_is_not_listed_for_sale_anymore() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(true, NftOpenForSale::contains_key(&context.nft_id));

                assert_ok!(NftManager::proxy(Origin::signed(context.relayer), call));

                assert_eq!(false, NftOpenForSale::contains_key(&context.nft_id));
            });
        }
        ...
        fn nft_nonce_is_increased() {
            ...
            ext.execute_with(|| {
                ...
                let original_nonce = NftManager::nfts(context.nft_id).nonce;

                assert_ok!(NftManager::proxy(Origin::signed(context.relayer), call));

                assert_eq!(
                    original_nonce + 1u64,
                    NftManager::nfts(context.nft_id).nonce
                );
            });
        }
    ...
    mod fails_when {
        ...
        fn origin_is_unsigned() {
            ...
            ext.execute_with(|| {
                ...
                assert_noop!(
                    NftManager::proxy(RawOrigin::None.into(), call.clone()),
                    BadOrigin
                );
            });
        }
        ...
        fn sender_is_not_owner() {
            ...
            ext.execute_with(|| {
                ...
                <NftManager as Store>::Nfts::mutate(context.nft_id, |nft| {
                    nft.owner = other_owner.account_id();
                });
        ...
        fn mismatched_proof_context() {
            ...
            ext.execute_with(|| {
                ...
                let call = Box::new(MockCall::NftManager(
                    super::Call::<TestRuntime>::signed_cancel_list_fiat_nft(proof, context.nft_id),
                ));
...
mod get_proof {
    ...
    fn fails_for_invalid_calls() {
        let mut ext = ExtBuilder::build_default().as_externality();
        ext.execute_with(|| {
            let invalid_call = MockCall::System(SystemCall::remark(vec![]));
