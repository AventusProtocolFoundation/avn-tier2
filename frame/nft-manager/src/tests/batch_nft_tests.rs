...
fn build_proof(signer: &AccountId, relayer: &AccountId, signature: Signature,) -> Proof<Signature, AccountId> {
    return Proof {
        signer: *signer,
        relayer: *relayer,
        signature: signature,
...
fn create_batch() -> U256 {
    ...
    let batch_id = generate_batch_id::<TestRuntime>(NextSingleNftUniqueId::get());

    assert_eq!(false, BatchInfoId::contains_key(batch_id));
    ...
    fn default() -> Self {
        ...
        let royalties = vec![
            Royalty {
                ...
                rate: RoyaltyRate {
                    parts_per_million: 1_000u32,
                },
            },
            Royalty {
                recipient_t1_address: H160(hex!("0000000000000000000000000000000000000003")),
                rate: RoyaltyRate {
                    parts_per_million: 500u32,
                },
            },
        ];

        CreateBatchContext {
            origin: Origin::signed(creator.account_id()),
...
impl CreateBatchContext {
    ...
    fn batch_created_event_emitted(&self, batch_id: U256) -> bool {
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::BatchCreated(
                    batch_id,
                    self.total_supply,
...
mod signed_create_batch {
    ...
    fn succeeds_with_good_parameters() {
        ...
        ext.execute_with(|| {
            let context = CreateBatchContext::default();
            let expected_batch_id = generate_batch_id::<TestRuntime>(NextSingleNftUniqueId::get());

            assert_eq!(false, BatchInfoId::contains_key(expected_batch_id));
            ...
            let info = <NftInfos<TestRuntime>>::get(BatchInfoId::get(expected_batch_id));
            ...
            assert_eq!(U256::zero() + 1, NextSingleNftUniqueId::get());
        });
    }
    ...
    mod fails_when {
        ...
        fn origin_is_unsigned() {
            ...
            ext.execute_with(|| {
                ...
                assert_noop!(
                    NftManager::proxy(RawOrigin::None.into(), inner_call),
                    BadOrigin
                );
            });
        }
            ...
            ext.execute_with(|| {
                ...
                context.royalties = vec![
                    ...
                    Royalty {
                        ...
                        rate: RoyaltyRate {
                            parts_per_million: ROYALTY_RATE_DENOMINATOR,
                        },
                    },
                ];
...
impl MintBatchNftContext {
    ...
    fn create_signed_mint_batch_nft_call(&self, batch_id: U256, index: u64) -> Box<<TestRuntime as Config>::Call> {
    ...
    fn create_signed_mint_batch_nft_proof(&self, batch_id: U256, index: u64) -> Proof<Signature, AccountId> {
        let data_to_sign = (
    ...
    fn mint_batch_nft_event_emitted(&self, batch_id: U256) -> bool {
        return System::events().iter().any(|a| {
            a.event == Event::nft_manager(crate::Event::<TestRuntime>::BatchNftMinted(
                    self.nft_id,
                    batch_id,
                    self.t1_authority,
                    self.nft_owner_account
...
mod signed_mint_batch_nft {
    ...
    mod succeeds {
        ...
        fn with_good_parameters() {
            ...
            ext.execute_with(|| {
                ...
                assert!(NftBatches::contains_key(&batch_id));

                // Newly minted nft is linked to the correct batch_id
                assert_eq!(NftBatches::get(batch_id)[0], context.nft_id);
                ...
                let info = <NftInfos<TestRuntime>>::get(BatchInfoId::get(batch_id));
                assert!(NftBatches::get(&batch_id).len() <= info.total_supply.try_into().unwrap());
            });
        }
        ...
        fn with_multiple_batch_nfts_minted() {
            ...
            ext.execute_with(|| {
                ...
                context.unique_external_ref = String::from("Offchain location of NFT 1").into_bytes();
                index = 1u64;
                ...
                assert_eq!(NftBatches::get(batch_id).len(), 2);

                // Ownership data is updated
                assert_eq!(true, nft_is_owned(&context.nft_owner_account, &NftBatches::get(batch_id)[0]));
                assert_eq!(true, nft_is_owned(&context.nft_owner_account, &NftBatches::get(batch_id)[1]));
            });
        }
    }
    ...
    mod fails_when {
        ...
        mod the_proof_is_invalid_because {
            ...
            fn get_call(context: &MintBatchNftContext, batch_id: &U256, index: &u64, data_to_sign: &[u8]) -> Box<MockCall> {
            ...
            fn create_batch_and_assert_success(context: &MintBatchNftContext, batch_id: U256, index: u64) {
               //now show that it will work if we fix the bad data.
               let data_to_sign = (
                    SIGNED_MINT_BATCH_NFT_CONTEXT,
                    &context.relayer,
            ...
            fn mismatched_proof_external_ref() {
                ...
                ext.execute_with(|| {
                    ...
                    let bad_external_ref = (context.unique_external_ref.clone(), String::from("bad_ref")).encode();
                    let data_to_sign = (
                        SIGNED_MINT_BATCH_NFT_CONTEXT,
                        &context.relayer,
                        &batch_id,
...
impl OpenForSaleContext {
    fn create_signed_list_batch_for_sale_call(&self, batch_id: U256, nonce: u64) -> Box<<TestRuntime as Config>::Call> {
    ...
    fn create_signed_list_batch_for_sale_proof(&self, batch_id: U256, nonce: u64) -> Proof<Signature, AccountId> {
        let data_to_sign = (
    ...
    fn batch_listed_for_sale_event_emitted(&self, batch_id: U256) -> bool {
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::BatchOpenForSale(
                    batch_id,
                    self.market,
                ))
        });
    ...
    mod succeeds {
        ...
        fn with_good_parameters_on_ethereum() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(true, BatchOpenForSale::contains_key(&batch_id));

                // The market is correctly set
                assert_eq!(BatchOpenForSale::get(&batch_id), context.market);
    ...
    mod fails_when {
        ...
        fn total_supply_is_exceeded() {
            ...
            ext.execute_with(|| {
                ...
                NftBatches::insert(batch_id, nft_ids);
        ...
        fn batch_already_listed() {
            ...
            ext.execute_with(|| {
                ...
                let mut inner_call = context.create_signed_list_batch_for_sale_call(batch_id, nonce);
        ...
        mod the_proof_is_invalid_because {
            ...
            fn get_call(context: &OpenForSaleContext, batch_id: &U256, data_to_sign: &[u8]) -> Box<MockCall> {
                let signature = sign(&context.creator_key_pair, data_to_sign);
                let proof = build_proof(&context.creator_account, &context.relayer, signature);
            ...
            fn create_batch_and_assert_success(context: &OpenForSaleContext, batch_id: U256, nonce: u64) {
                //now show that it will work if we fix the bad data.
                let data_to_sign = (
                    SIGNED_LIST_BATCH_FOR_SALE_CONTEXT,
                    &context.relayer,
...
impl EndBatchSaleContext {
    fn create_signed_end_batch_sale_call(&self, batch_id: U256, nonce: u64) -> Box<<TestRuntime as Config>::Call> {
    ...
    fn create_signed_end_batch_sale_proof(&self, batch_id: U256, nonce: u64) -> Proof<Signature, AccountId> {
        let data_to_sign = (
    ...
    fn batch_sale_ended_event_emitted(&self, batch_id: U256) -> bool {
        return System::events().iter().any(|a| {
            a.event
                == Event::nft_manager(crate::Event::<TestRuntime>::BatchSaleEnded(
                    batch_id,
                    self.market,
                ))
        });
...
mod signed_end_batch_sale {
    ...
    mod succeeds {
        ...
        fn with_good_parameters() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(false, BatchOpenForSale::contains_key(&batch_id));
    ...
    mod fails_when {
        ...
        mod the_proof_is_invalid_because {
            ...
            fn get_call(context: &EndBatchSaleContext, batch_id: &U256, data_to_sign: &[u8]) -> Box<MockCall> {
                let signature = sign(&context.creator_key_pair, data_to_sign);
                let proof = build_proof(&context.creator_account, &context.relayer, signature);
            ...
            fn end_sale_and_assert_success(context: &EndBatchSaleContext, batch_id: U256, nonce: u64) {
                //now show that it will work if we fix the bad data.
                let data_to_sign = (
                    SIGNED_END_BATCH_SALE_CONTEXT,
                    &context.relayer,
