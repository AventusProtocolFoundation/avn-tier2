...
use crate::tests_eth_transaction_type::*;
...
fn ecdsa_signature_to_vec_works() {
    ...
    let sig = EcdsaSignature { r: r, s: s, v: v };
...
// TODO [TYPE: tests][PRI: low]: test abi encoding --> this looks like it is the general encoding function
// I think it is best to test the public facing functions that call this one instead,
...
fn short_ethereum_description_appends_to_original_description() {
    ...
    assert_eq!(
        &values[number_of_original_terms],
        &Token::Uint(tx_id_be.into())
    );

    assert_eq!(
        values[number_of_original_terms + 1],
        Token::FixedBytes(from.to_vec())
    );
    assert_eq!(
        number_of_terms - number_of_original_terms,
        expected_new_arguments.len()
    );
}
...
fn full_ethereum_description_appends_signatures() {
    ...
    assert_eq!(
        values[number_of_original_terms],
        Token::Bytes(signatures.to_bytes())
    );
...
fn adding_duplicate_signatures_should_fail() {
    ...
    assert_err!(
        signatures.add(same_signature),
        OtherError::DuplicateSignature
    );
}
