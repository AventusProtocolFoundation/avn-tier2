...
// It has been generated using web3.js, using mock-T1/api/CreateAbiTestData.js script in lydia-common
...
fn generate_merkle_root_mock_data_with_signatures(
    ...
    sigs.signatures_list
        .append(&mut create_mock_ecdsa_signatures());
...
fn generate_deregister_validator_mock_data_with_signatures() -> EthTransactionCandidate {
    let (mut sigs, mut candidate_transaction) = generate_deregister_validator_mock_data_with_sender();
    sigs.signatures_list.append(&mut create_mock_ecdsa_signatures());
    candidate_transaction.signatures = sigs.clone();
    return candidate_transaction;
}

fn generate_deregister_validator_mock_data_with_sender(
) -> (EthSignatures, EthTransactionCandidate) {
    ...
    let candidate_transaction = EthTransactionCandidate::new(
        ...
        EthTransactionType::DeregisterValidator(DeregisterValidatorData::new(t2_public_key.clone())),
        DEFAULT_QUORUM,
    );
...
fn generate_merkle_root_mock_data_without_sender(
    ...
    // TODO [TYPE: test refactoring][PRI: medium]: move this `from` to `generate_merkle_root_mock_data_with_sender`
...
fn create_mock_ecdsa_signatures() -> Vec<ecdsa::Signature> {
    vec![
        ecdsa::Signature::from_raw([4;65]),
        ecdsa::Signature::from_raw([5;65]),
    ]
}
...
fn mark_avn_tx_as_sent(tx_id: TransactionId, eth_tx_hash: H256) {
    assert_ok!(<EthereumTransactions as Store>::Repository::mutate(
        tx_id,
        |tx| { tx.set_eth_tx_hash::<TestRuntime>(eth_tx_hash) }
    ));
}
...
fn validate_generated_publish_root_bytes_abi_success() {
    ...
    ext.execute_with(|| {
        ...
        let tx = candidate_tx
            .to_abi(EthereumTransactions::get_publish_root_contract())
            .unwrap();
...
impl DefaultTransactionBuilder {
    ...
    pub fn as_candidate(&self, tx_id: TransactionId) -> EthTransactionCandidate {
        // Returns an EthTransactionCandidate that should be equal to what the Module produces when submitting the tx_type
        // The module obtains the tx_id by reading the nonce by calling get_unique_transaction_identifier()
        // If we call this here, then the module will always generate a different tx_id
        // Therefore, this function does not use the same method, to avoid modifying the test state
    ...
    mod updates_unique_identifier {
        ...
        fn fails_if_transaction_already_reserved() {
            ...
            ext.execute_with(|| {
                ...
                let old_unique_identifier = EthereumTransactions::get_current_unique_transaction_identifier();
                ...
                assert_err!(
                    EthereumTransactions::reserve_transaction_id(
                        &transaction.as_type()
                    ),
        ...
        fn when_call_succeeds() {
            ...
            ext.execute_with(|| {
                ...
                assert_ok!(EthereumTransactions::reserve_transaction_id(
                    &transaction.as_type()
                ));
...
fn call_submit_candidate_for_tier1(transaction: EthTransactionCandidate, sender: AccountId) -> DispatchResult {
    ...
    mod fails_when {
        ...
        fn transaction_exists_in_repository() {
            ...
            ext.execute_with(|| {
                ...
                EthereumTransactions::insert_to_repository(
        ...
        fn other_than_reserved_id_is_used() {
            ...
            ext.execute_with(|| {
                ...
                let candidate = context.transaction
                    .with_sender(context.from)
                    .with_confirmations()
                    .as_candidate(context.tx_id);
    ...
    mod succeeds_and {
        ...
        fn nonce_does_not_change() {
            ...
            ext.execute_with(|| {
                ...
                assert_eq!(old_unique_identifier, EthereumTransactions::get_current_unique_transaction_identifier());
            });
        }
        ...
        fn event_is_emitted() {
            ...
            ext.execute_with(|| {
                ...
                let event = mock::Event::ethereum_transactions(
                    crate::Event::<TestRuntime>::TransactionReadyToSend(context.tx_id, context.from)
                );
...
mod set_eth_tx_hash_for_dispatched_tx {
    ...
    fn setup() -> Context {
        ...
        Context {
            tx_id: tx_id,
            submitter: submitter,
            hash: H256::from_slice(&DEFAULT_HASH),
        }
    }
    ...
    mod fails_when {
        ...
        fn origin_is_signed() {
            ...
            ext.execute_with(|| {
                ...
                // I have checked that the error is returned, but not that the state remains untouched
                assert_noop!(
...
fn eth_transaction_candidate_set_eth_tx_hash_success() {
    ...
    assert!(test_tx
        .set_eth_tx_hash::<TestRuntime>(new_eth_tx_hash)
        .is_ok());
}
...
fn tx_ready_to_be_sent_returns_correct_value_when_more_than_max_transactions_exist() {
    ...
    ext.execute_with(|| {
        ...
        let (from, candidate_transactions) =
            generate_valid_dispatched_transactions(
                MAX_VALUES_RETURNED + 2,
...
fn tx_ready_to_be_sent_excludes_already_sent() {
    ...
    ext.execute_with(|| {
        ...
        let (from, candidate_transactions) = generate_valid_dispatched_transactions(count_of_tx_to_sent, candidate_transaction);
        assert_eq!(candidate_transactions.len(), count_of_tx_to_sent);

        let txs_to_send = EthereumTransactions::transactions_ready_to_be_sent(&from);
        assert_eq!(txs_to_send.len(), get_expected_count_for_transactions_to_send(count_of_tx_to_sent));
...
fn tx_ready_to_be_sent_only_includes_own_transactions() {
    ...
    ext.execute_with(|| {
        let (_, _, _, candidate_transaction) = generate_merkle_root_mock_data_with_signatures();
        let (from, candidate_transactions) = generate_valid_dispatched_transactions(1, candidate_transaction);
        ...
        assert_eq!(txs_to_send.len(), 1);
        assert_eq!(
            txs_to_send[0].1.from,
            to_32_bytes(&SELECTED_SENDER_ACCOUNT_ID)
        );
...
fn eth_tx_hash_validates_correctly() {
    ...
    ext.execute_with(|| {
        let (_, _, _, candidate_transaction) = generate_merkle_root_mock_data_with_signatures();
        let (_, candidate_transactions) = generate_valid_dispatched_transactions(1, candidate_transaction);
...
fn test_get_contract_address_for_eth_txn(transaction_type: &EthTransactionType, expected_contract_address: H160) {
    let actual_contract_address =
        EthereumTransactions::get_contract_address(transaction_type);

    assert!(
        actual_contract_address.is_some(),
        "Contract address must not be empty"
    );
    assert_eq!(
        actual_contract_address.unwrap(),
...
fn eth_get_contract_address_works_with_valid_input() {
    ...
    ext.execute_with(|| {
        ...
        let (_, dispatched_candidate_txs) = generate_valid_dispatched_transactions(1, publish_root_candidate_tx);
        ...
        let deregister_validator_candidate_tx = generate_deregister_validator_mock_data_with_signatures();
        let (_, dispatched_candidate_txs) = generate_valid_dispatched_transactions(1, deregister_validator_candidate_tx);
    ...
    mod succeeds_when {
        ...
        fn origin_is_root_and_value_exists()  {
            ...
            ext.execute_with(||{
                ...
                assert_eq!(true, <ReservedTransactions>::contains_key(&context.transaction_type));
                assert_ok!(EthereumTransactions::unreserve_transaction(RawOrigin::Root.into(), context.transaction_type.clone()));
                assert_eq!(false, <ReservedTransactions>::contains_key(&context.transaction_type));
                assert_eq!(true, <ReservedTransactions>::contains_key(EthTransactionType::Discarded(context.tx_id)));
        ...
        fn origin_is_root_and_value_does_not_exist()  {
            ...
            ext.execute_with(||{
                ...
                assert_ok!(EthereumTransactions::unreserve_transaction(RawOrigin::Root.into(), context.transaction_type.clone()));
                assert_eq!(false, <ReservedTransactions>::contains_key(&context.transaction_type));
                assert_eq!(false, <ReservedTransactions>::contains_key(EthTransactionType::Discarded(context.tx_id)));
            });
        }
    ...
    mod fails_when {
        ...
        fn origin_is_unsigned()  {
            ...
            ext.execute_with(||{
                ...
                assert_eq!(true, <ReservedTransactions>::contains_key(&context.transaction_type));
                assert_noop!(EthereumTransactions::unreserve_transaction(RawOrigin::None.into(), context.transaction_type.clone()), BadOrigin);
                assert_eq!(true, <ReservedTransactions>::contains_key(&context.transaction_type));
                assert_eq!(false, <ReservedTransactions>::contains_key(EthTransactionType::Discarded(context.tx_id)));
            });
        }
        ...
        fn origin_is_signed()  {
            ...
            let context: Context = Default::default();
                ...
                assert_eq!(true, <ReservedTransactions>::contains_key(&context.transaction_type));
                assert_noop!(EthereumTransactions::unreserve_transaction(Origin::signed(Default::default()), context.transaction_type.clone()), BadOrigin);
                assert_eq!(true, <ReservedTransactions>::contains_key(&context.transaction_type));
                assert_eq!(false, <ReservedTransactions>::contains_key(EthTransactionType::Discarded(context.tx_id)));
            });
        }
