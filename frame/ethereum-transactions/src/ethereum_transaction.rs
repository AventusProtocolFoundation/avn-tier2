...
pub struct EthTransactionCandidate {
    ...
    // TODO [TYPE: business logic][PRI: high] this needs review since we allow locks to expire, it is possible to send the same transaction twice and
    // then we need to update this value.
    // Example: we create a lock to send a transaction to Ethereum. We send it and receive a temporary `eth_tx_hash`
    // The transaction later becomes stale and has to be resent.
    // But because tx_hash can only be set once, we will not be able to update it as is.

    // eth_tx_hash should be None and set only once, with an ethereum transaction hash.EthTransactionCandidate
    // Should we run in a situation that we try to overwrite a value, then an error should be raised, logged and investigated
    // To enforce this, we make this a private field with set/get functions that will throw errors.
    // Additionally since we can't have a Option<H256> in Storage, we map H256::zero() with None in the get function.
...
impl EthTransactionCandidate {
    ...
    // TODO [TYPE: refactoring][PRI: medium]: We have multiple `to_abi()` methods that return different things, we should rename this one to `to_abi_encoding()`
...
// Avn compatible ethereum methods must take a `_t2TransactionId` and `_confirmations` as the last 2 parameters
// `_confirmations` is a vector of signatures of all the method parameters followed by `_t2TransactionId` and `_t2PublicKey`

impl EthAbiHelper {
    // TODO [TYPE: refactoring][PRI: medium]: migrate this function to avn-common, together with its tests
    ...
    pub fn generate_full_ethereum_description(
        ...
        mut_call
            .call_values
            .push(Token::Bytes(signatures.to_bytes()));
        Ok(mut_call)
    }
...
// Ethereum-like ECDSA signatures are 65 bytes long. std::array::LengthAtMost32 trait does now allow us to have
// the signatures stored as bytes array, so we are encapsulating this in a struct
#[derive(Encode, Decode, Clone, PartialEq, Eq, Debug, Default)]
...
impl EcdsaSignature {
    pub fn new(raw_sig: [u8; 65]) -> Result<EcdsaSignature, OtherError> {
        ...
        return Ok(EcdsaSignature { r: r, s: s, v: v });
    }
...
impl EthSignatures {
    ...
    pub fn add(&mut self, signature: ecdsa::Signature) -> Result<(), OtherError> {
        ...
        // but does this have a hit in performance that will be caught later or is this checked elsewhere?
    ...
    pub fn new() -> Self {
        return EthSignatures {
            signatures_list: Vec::<ecdsa::Signature>::new(),
        };
    }
}
