//! # Aventus Node Pallet
//!
//! This pallet provides functionality that is common for other avn pallets such as handling offchain worker
//! validations, managing a list of validator accounts and their signing keys.
//!
//! The Authority defined here will also be shared by the other pallets that depend on AVN. This means
//! there will be 1 signing key for all AVN pallets (Ethereum-events, Ethereum-transactions...). This key is separate from
//! the rest of the session keys.
//!
//! From a security point of view, the rationale to implement it this way is because if one of the signing keys is compromised,
//! we dont want to trust that node for any other actions they carry out so by having 1 key for our AVN pallets, we don't make our
//! system less secure.
...
use sp_std::{prelude::*};
use frame_support::{decl_storage, decl_module, decl_error, debug, Parameter, dispatch::DispatchResult, traits::OneSessionHandler};
...
use sp_avn_common::{
    ...
    recover_public_key_from_ecdsa_signature, EXTERNAL_SERVICE_PORT_NUMBER_KEY, DEFAULT_EXTERNAL_SERVICE_PORT_NUMBER
};
...
decl_storage! {
	trait Store for Module<T: Config> as Avn {
        /// The current set of validators (address and key) that may issue a transaction from the offchain worker.
        pub Validators get(fn validators): Vec<Validator<T::AuthorityId, T::AccountId>>;
	}
}
...
impl<T: Config> Module<T> {
    ...
    pub fn pre_run_setup(block_number: T::BlockNumber, caller_id: Vec<u8>)
        -> Result<Validator<T::AuthorityId, T::AccountId>, DispatchError>
    {
        ...
        OcwLock::record_block_run(block_number, caller_id).map_err(|e|
        match e {
            OcwStorageError::OffchainWorkerAlreadyRun => {
                debug::native::info!("** Offchain worker has already run for block number: {:?}", block_number);
                Error::<T>::OffchainWorkerAlreadyRun
            },
            OcwStorageError::ErrorRecordingOffchainWorkerRun => {
                debug::native::error!("** Unable to record offchain worker run for block {:?}, skipping", block_number);
    ...
    pub fn is_primary(block_number: T::BlockNumber, current_validator: &T::AccountId) -> Result<bool, Error<T>> {
        let primary_validator = Self::calculate_primary_validator(block_number)?;
        return Ok(&primary_validator == current_validator);
    }

    pub fn calculate_primary_validator(block_number: T::BlockNumber) -> Result<T::AccountId, Error<T>> {
    ...
    pub fn get_validator_for_current_node() -> Option<Validator<T::AuthorityId, T::AccountId>> {
        ...
        return validators.into_iter()
            .enumerate()
            .filter_map(move |(_, validator)| {
                local_keys.binary_search(&validator.key)
                .ok()
                .map(|_| validator)
            })
            .nth(0);
    ...
    pub fn post_data_to_service(url_path: String, post_body: Vec<u8>) -> Result<Vec<u8>, DispatchError> {
    ...
    pub fn eth_signature_is_valid(
        data: String,
        validator: &Validator<T::AuthorityId, T::AccountId>,
        signature: &ecdsa::Signature) -> bool
    {
        // verify that the incoming (unverified) pubkey is actually a validator
        if !Self::is_validator(&validator.account_id) {
            return false;
        ...
        match T::EthereumPublicKeyChecker::get_validator_for_eth_public_key(&recovered_public_key.expect("Checked for error")) {
    ...
    pub fn try_get_validator(account_id: &T::AccountId) -> Option<Validator<T::AuthorityId, T::AccountId>> {
    ...
    pub fn remove_validator_from_active_list(validator_id: &T::AccountId) {
        <Validators<T>>::mutate(|active_validators| {
            if let Some(validator_index) = active_validators.iter().position(|v| &v.account_id == validator_id) {
    ...
    pub fn get_external_service_port_number() -> String {
        let stored_value: Option<Option<Vec<u8>>> = StorageValueRef::persistent(EXTERNAL_SERVICE_PORT_NUMBER_KEY).get();
        ...
        if let Err(e) = port_number {
            debug::native::trace!("❌ External service port {} is not formatted correctly. Using default port.", e);
        ...
        if port_number.parse::<u32>().is_err() {
            debug::native::trace!("❌ External service port {} is not a valid number. Using default port.", port_number);
            return DEFAULT_EXTERNAL_SERVICE_PORT_NUMBER.into();
        }
    ...
    fn invoke_external_service(request: http::Request<Vec<Vec<u8>>>, url_path: String) -> Result<Vec<u8>, DispatchError> {
...
impl<T: Config> OneSessionHandler<T::AccountId> for Module<T> {
    ...
    {
        let avn_validators = validators.map(|x| Validator::new(x.0.clone(), x.1)).collect::<Vec<_>>();
        ...
        where I: Iterator<Item=(&'a T::AccountId, T::AuthorityId)>
    {
        ...
        T::NewSessionHandler::on_new_session(changed, &active_avn_validators, &disabled_avn_validators);
    }
...
pub trait AccountToBytesConverter<AccountId: Decode + Encode> {
    ...
    /// This function expects valid bytes that can be converted into an accountId. No validation is done here.
    fn try_from_any(bytes: Vec<u8>) -> Result<AccountId, DispatchError>;
}

impl<T: Config> AccountToBytesConverter<T::AccountId> for Module<T> {
    ...
    fn try_from_any(bytes: Vec<u8>) -> Result<T::AccountId, DispatchError> {
        ...
        return T::AccountId::decode(&mut &account_bytes[..]).map_err(|_| DispatchError::Other("Error converting to AccountId"));
    }
}
...
impl<AccountId: Member> EthereumPublicKeyChecker<AccountId> for () {
	fn get_validator_for_eth_public_key(_eth_public_key: &ecdsa::Public) -> Option<AccountId> { None }
}
...
impl<AccountId: Member> DisabledValidatorChecker<AccountId> for () {
	fn is_disabled(_validator_account_id: &AccountId) -> bool { false }
}
...
impl<BlockNumber: Member> FinalisedBlockChecker<BlockNumber> for () {
    fn is_finalised(_block_number: BlockNumber) -> bool { false }
}
