...
mod initial_lifts {
    ...
    impl Context {
        pub fn create() -> Self {
            ...
            let initial_lifts = INITIAL_LIFTS.iter().map(|x|{
    ...
    mod are_populated_when {
        ...
        fn genesis_config_has_some(){
            let mut ext = ExtBuilder::build_default()
                .with_genesis_and_initial_lifts()
                .as_externality();
            ext.execute_with(||{
...
mod initial_processed_events {
    ...
    impl Context {
        pub fn create() -> Self {
            let initial_processed_events = create_initial_processed_events().iter().map(|(x,_)| {
                x.clone()
            }).collect::<Vec<EthEventId>>();
            return Context{
                initial_processed_events: initial_processed_events
            };
        }
    }
    ...
    mod are_empty_when {
        ...
        fn genesis_config_has_none(){
            let mut ext = ExtBuilder::build_default()
                ...
                .as_externality();
            ext.execute_with(||{
                assert_eq!(<ProcessedEvents>::iter().count(), 0);
            });
        }
    }
    ...
    mod are_populated_when {
        ...
        fn genesis_config_has_some(){
            ...
            ext.execute_with(||{
                let context = Context::create();
                assert_eq!(<ProcessedEvents>::iter().count(), INITIAL_PROCESSED_EVENTS.len());
                for event in context.initial_processed_events {
                    assert_eq!(<ProcessedEvents>::contains_key(event), true);
                }
            });
        }
    }
