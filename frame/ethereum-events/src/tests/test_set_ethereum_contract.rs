...
mod test_set_ethereum_contract {
    ...
    impl Context {
        fn dispatch_set_ethereum_contract(&self) -> DispatchResult {
            return EthereumEvents::set_ethereum_contract(self.origin.clone(), self.ethereum_contract.clone(), self.new_contract_address.clone());
        }
    }
    ...
    mod successful_cases {
        ...
        fn update_lifting_contract() {
            ...
            ext.execute_with(||{
                let context: Context = Context {
                    ethereum_contract: EthereumContracts::Lifting,
                    ..Default::default()
                };
                assert_ne!(
                    context.new_contract_address,
        ...
        fn update_nft_marketplace_contract_for_reserved_marketplace() {
            ...
            ext.execute_with(||{
                ...
                assert_eq!(false, NftT1Contracts::contains_key(context.new_contract_address));

                assert_ok!(context.dispatch_set_ethereum_contract());
                assert_eq!(true, NftT1Contracts::contains_key(context.new_contract_address));
            });
        }
    }
    ...
    mod fails_when {
        ...
        fn zero_contract_should_fail() {
            ...
            ext.execute_with(||{
                let context: Context = Context {
                    new_contract_address: H160::zero(),
                    ..Default::default()
                };

                assert_noop!(context.dispatch_set_ethereum_contract(), Error::<TestRuntime>::InvalidContractAddress);
                assert_ne!(
                    context.new_contract_address,
                    EthereumEvents::get_contract_address_for_non_nft_event(&ValidEvents::AddedValidator).unwrap()
                );
