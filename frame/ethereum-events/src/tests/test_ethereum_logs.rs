...
mod test_add_validator_log {
    ...
    fn success_case() {
        ...
        ext.execute_with(||{
            ...
            assert_ok!(EthereumEvents::add_validator_log(Origin::signed(account_id_0()), tx_hash));
            ...
            assert!(EthereumEvents::unchecked_events().contains(&(validator_event.clone(), FIRST_INGRESS_COUNTER, 1)));

            let event = mock::Event::pallet_ethereum_events(
                crate::Event::<TestRuntime>::EthereumEventAdded(
                    validator_event.clone(),
                    account_id_0(),
                    EthereumEvents::get_contract_address_for_non_nft_event(&ValidEvents::AddedValidator).unwrap()
                ));
    ...
    mod successive_events {
        ...
        fn create_two_successive_events(signer: AccountId) -> (EthEventId, EthEventId) {
            ...
            assert_ok!(EthereumEvents::add_validator_log(Origin::signed(signer), tx_hash));
            let validator_event_1 = EthEventId {
                signature: ValidEvents::AddedValidator.signature(),
                transaction_hash: tx_hash,
            };

            assert_ok!(EthereumEvents::add_validator_log(Origin::signed(signer), second_tx_hash));
        ...
        fn have_consecutive_ingress_counters() {
            ...
            ext.execute_with(||{
                ...
                let (validator_event_1, validator_event_2) = create_two_successive_events(account_id_0());

                assert_eq!(EthereumEvents::unchecked_events().len(), 2);
                assert!(EthereumEvents::unchecked_events().contains(&(validator_event_1.clone(), DEFAULT_INGRESS_COUNTER + 1, 1)));
                assert!(EthereumEvents::unchecked_events().contains(&(validator_event_2.clone(), DEFAULT_INGRESS_COUNTER + 2, 1)));
            });
        }
    ...
    fn zero_hash_should_fail() {
        ...
        ext.execute_with(||{
            ...
            assert_noop!(EthereumEvents::add_validator_log(Origin::signed(Default::default()), tx_hash_invalid), Error::<TestRuntime>::MalformedHash);
    ...
    fn unsigned_origin_should_fail() {
        ...
        ext.execute_with(||{
            let tx_hash: H256 = H256::random();
            assert_eq!(System::events(), vec![]);
            assert_noop!(EthereumEvents::add_validator_log(RawOrigin::None.into(), tx_hash), BadOrigin);
            // Ensure no events were emitted in avn
            assert_eq!(System::events(), vec![]);
        });
    }
...
mod test_add_lift_log {
    ...
    fn success_case () {
        ...
        ext.execute_with(||{
            let tx_hash: H256 = H256::random();
            assert_ok!(EthereumEvents::add_lift_log(Origin::signed(account_id_0()), tx_hash));
            let lift_event = EthEventId {
                signature: ValidEvents::Lifted.signature(),
                transaction_hash: tx_hash,
            };
            assert_eq!(EthereumEvents::unchecked_events().len(), 1);
            assert!(EthereumEvents::unchecked_events().contains(&(lift_event.clone(), FIRST_INGRESS_COUNTER, 1)));
    ...
    fn zero_hash_should_fail() {
        ...
        ext.execute_with(||{
            let tx_hash_invalid: H256 = H256::zero();
            assert_noop!(EthereumEvents::add_lift_log(Origin::signed(Default::default()), tx_hash_invalid), Error::<TestRuntime>::MalformedHash);
            // Ensure no events were emitted in avn
            assert_eq!(System::events(), vec![]);
        });
    }
    ...
    fn duplicate_event_should_fail() {
        ...
        ext.execute_with(||{
            let tx_hash: H256 = H256::random();
            EthereumEvents::insert_to_unchecked_events( &EthEventId{
                signature: ValidEvents::Lifted.signature(),
                transaction_hash: tx_hash.clone(),
            }
            ,DEFAULT_INGRESS_COUNTER);
            assert_noop!(EthereumEvents::add_lift_log(Origin::signed(Default::default()), tx_hash), Error::<TestRuntime>::DuplicateEvent);
            // Ensure no events were emitted in avn
            assert_eq!(System::events(), vec![]);
        });
    }
...
mod test_add_ethereum_log {
    ...
    impl Context {
        ...
        fn dispatch_add_ethereum_log(&self) -> DispatchResult {
            return EthereumEvents::add_ethereum_log(self.origin.clone(), self.nft_event_type.clone(), self.tx_hash);
        }
    }
    ...
    fn perform_success_case(event_type: ValidEvents) {
        ...
        ext.execute_with(||{
            ...
            let context: Context = Context {
                nft_event_type: event_type,
                ..Default::default()
            };

            assert_ok!(EthereumEvents::add_ethereum_log(context.origin.clone(), context.nft_event_type.clone(), context.tx_hash));
            ...
            if context.nft_event_type.is_nft_event() {
                event = mock::Event::pallet_ethereum_events(
                ...
                event = mock::Event::pallet_ethereum_events(
                    crate::Event::<TestRuntime>::EthereumEventAdded(
                        ethereum_event.clone(),
                        account_id_0(),
                        EthereumEvents::get_contract_address_for_non_nft_event(&context.nft_event_type).unwrap()
                ));
            }
    ...
    mod successive_events {
        ...
        fn create_two_successive_events(signer: AccountId, first_event_type: ValidEvents, second_event_type: ValidEvents) -> (EthEventId, EthEventId) {
            let first_tx_hash: H256 = H256::from([1u8; 32]);
            let second_tx_hash: H256 = H256::from([2u8; 32]);

            assert_ok!(EthereumEvents::add_ethereum_log(Origin::signed(signer), first_event_type.clone(), first_tx_hash));
            let validator_event_1 = EthEventId {
                signature: first_event_type.signature(),
                transaction_hash: first_tx_hash,
            };

            assert_ok!(EthereumEvents::add_ethereum_log(Origin::signed(signer), second_event_type.clone(), second_tx_hash));
            let validator_event_2 = EthEventId {
        ...
        fn have_consecutive_ingress_counters() {
            ...
            ext.execute_with(||{
                EthereumEvents::set_ingress_counter(DEFAULT_INGRESS_COUNTER);
                let (validator_event_1, validator_event_2) = create_two_successive_events(account_id_0(), ValidEvents::NftMint, ValidEvents::NftMint);
        ...
        fn generate_several_system_events() {
            ...
            ext.execute_with(||{
                EthereumEvents::set_ingress_counter(DEFAULT_INGRESS_COUNTER);
                let (validator_event_1, validator_event_2) = create_two_successive_events(account_id_0(),ValidEvents::NftCancelListing, ValidEvents::NftCancelListing);
    ...
    fn zero_hash_should_fail() {
        let mut ext = ExtBuilder::build_default().with_genesis_config().as_externality();
        ext.execute_with(||{
            let invalid_context: Context = Context {
                tx_hash: H256::zero(),
                ..Default::default()
            };

            assert_noop!(
                EthereumEvents::add_ethereum_log(invalid_context.origin, ValidEvents::NftMint, invalid_context.tx_hash),
    ...
    fn duplicate_event_should_fail() {
        ...
        ext.execute_with(||{
            ...
            assert_noop!(
                context.dispatch_add_ethereum_log(),
                Error::<TestRuntime>::DuplicateEvent
            );
            // Ensure no events were emitted in avn
            assert_eq!(0, System::events().len());
...
mod add_event {
    ...
    fn valid_lift() {
        ...
        ext.execute_with(||{
            ...
            assert_ok!(EthereumEvents::add_event(ValidEvents::Lifted, tx_lift_hash.clone(), account_id ));
            let lift_event = EthEventId {
                signature: ValidEvents::Lifted.signature(),
    ...
    fn valid_validator() {
        ...
        ext.execute_with(||{
            ...
            let account_id = account_id_1();
            assert_ok!(EthereumEvents::add_event(ValidEvents::AddedValidator, tx_validator_hash.clone(), account_id));
            let validator_event = EthEventId {
                signature: ValidEvents::AddedValidator.signature(),
    ...
    fn existing_event_should_fail() {
        ...
        ext.execute_with(||{
            ...
            assert_noop!(EthereumEvents::add_event(ValidEvents::AddedValidator, event_id.transaction_hash, account_id), Error::<TestRuntime>::DuplicateEvent);
            // Ensure no events were emitted in avn
            assert_eq!(System::events(), vec![]);
        });
    }
