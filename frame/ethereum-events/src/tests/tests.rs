...
fn test_compute_result_ok() {
    ...
    ext.execute_with(||{
        ...
        let result = EthereumEvents::compute_result(block_number, Ok(json), unchecked_event, &account_id_1());
        ...
        // ready_for_processing_at_block value should not be set here, so we skip the checks.event_topics
        // It should be set after the offchain worker finishes the tasks and re-enters as a new Tx to the system
...
mod signature_in {
    ...
    mod submit_checkevent_result {
        ...
        fn setup() -> Context {
            ...
            let validator = prepare_to_invoke_function(
        ...
        fn is_correctly_validated() {
            ...
            ext.execute_with(||{
                ...
                simulate_http_response(&offchain_state, &context.event_id, GOOD_STATUS, GOOD_BLOCK_CONFIRMATIONS);
                ...
                match tx.call {
                    Call::EthereumEvents(inner_tx) => { assert_ok!(EthereumEvents::validate_unsigned(TransactionSource::Local, &inner_tx)); },
                    _ => unreachable!()
                }
            });
        }

        // Note: the previous test guarantees that validate_unsigned is aligned to the signatures produced in the code
        // This test guarantees that the signed data includes the fields we want
        // This could be made clearer with more tests, one testing each field, but for now this will do
        #[test]
        fn includes_all_relevant_fields() {
            ...
            ext.execute_with(||{
                ...
                match tx.call {
                    Call::EthereumEvents(crate::Call::submit_checkevent_result(result, counter, signature, validator)) => {
    ...
    mod challenge_event {
        ...
        fn includes_all_relevant_fields() {
            ...
            ext.execute_with(||{
                ...
                match tx.call {
                    Call::EthereumEvents(crate::Call::challenge_event(challenge, counter, signature, validator)) => {
    ...
    mod process_event {
        ...
        fn includes_all_relevant_fields() {
            ...
            ext.execute_with(||{
                ...
                match tx.call {
                    Call::EthereumEvents(crate::Call::process_event(event_id, counter, validator, signature)) => {
