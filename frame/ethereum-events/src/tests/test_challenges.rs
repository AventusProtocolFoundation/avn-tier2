...
use frame_support::{assert_ok, assert_err, assert_noop, dispatch, unsigned::ValidateUnsigned};
...
fn get_event_check_result(id: &EthEventId, data: &EventData, result: CheckResult) ->
    EthEventCheckResult<
        <TestRuntime as system::Config>::BlockNumber, AccountId>
{
...
fn create_challenge(id: EthEventId, reason: ChallengeReason, challenger: AccountId) -> Challenge<AccountId>
{
...
fn mock_send_challenge_transaction_from_ocw(
    ...
    return EthereumEvents::challenge_event(RawOrigin::None.into(), challenge, ingress_counter, signature, validator);
}
...
// TODO [TYPE: test refactoring][PRI: low]: Review these tests to work with the builder's pattern setup
// Currently, a straightforward substitution makes several of these tests fail
#[test]
// * when pending challenge queue is empty
...
#[test]
// * when pending challenge queue is not empty and the first event can be validated but the second one is checked by us
fn test_get_event_to_validate_first_good_event() {
	with_offchain_worker(eth_events_test_with_validators()).execute_with(|| {
        ...
        let (next_event_to_validate, counter, _) = EthereumEvents::get_next_event_to_validate(&this_validator_account_id).unwrap();
...
#[test]
// * when pending challenge queue is not empty and event 2 can be validated by `this_validator_account_id` and
...
fn test_get_event_to_validate_mixed() {
	with_offchain_worker(eth_events_test_with_validators()).execute_with(|| {
        ...
        let (next_event_to_validate, counter, _) = EthereumEvents::get_next_event_to_validate(&new_validator_account_id).unwrap();
        let expected_event_id = EthEventId {
            signature: ValidEvents::AddedValidator.signature(),
...
fn test_challenge_validation_match_check() {
    ...
    ext.execute_with(||{
        ...
        let challenge = EthereumEvents::get_challenge_if_required(checked, validated, validator_account_id);
...
fn test_challenge_invalid_same_event_data() {
    ...
    ext.execute_with(||{
        ...
        let checked = get_event_check_result(&event_id, &EventData::EmptyEvent, CheckResult::Invalid);
        let validated = get_event_check_result(&event_id, &EventData::EmptyEvent, CheckResult::Invalid);
...
fn test_challenge_invalid_different_event_data() {
    ...
    ext.execute_with(||{
        ...
        let checked = get_event_check_result(&event_id, &EventData::EmptyEvent, CheckResult::Invalid);
        let validated = get_event_check_result(&event_id, &add_validator_data, CheckResult::Invalid);
...
fn test_challenge_missing_event() {
	eth_events_test_with_validators().execute_with(|| {
        ...
        let challenge = create_challenge(bad_event_id, ChallengeReason::IncorrectResult, account_id_1());
        let signature = validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, challenge.clone()).encode()).unwrap();

        assert_eq!(EthereumEvents::events_pending_challenge().len(), 0);
        assert_noop!(
            EthereumEvents::challenge_event(RawOrigin::None.into(), challenge, DEFAULT_INGRESS_COUNTER, signature, validator),
...
fn test_challenge_out_of_challenge_window() {
	eth_events_test_with_validators().execute_with(|| {
        ...
        let challenge = create_challenge(EthereumEvents::get_event_id(0), ChallengeReason::IncorrectResult, validator.account_id);
        let signature = validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, challenge.clone()).encode()).unwrap();
...
fn test_challenge_invalid_validator() {
	eth_events_test_with_validators().execute_with(|| {
        ...
        let bad_validator = Validator::new(TestAccount::new([10u8; 32]).account_id(), UintAuthorityId(10));
        let challenge = create_challenge(EthereumEvents::get_event_id(0), ChallengeReason::IncorrectResult, bad_validator.account_id);
        let signature = bad_validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, challenge.clone()).encode()).unwrap();
...
fn test_challenge_invalid_signature() {
	eth_events_test_with_validators().execute_with(|| {
        ...
        // Note: the implementation of sign() on UintAuthorityId will return the same signature for different signer
        // as long as the data being signed is the same.
        let bad_signature = validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, bad_data_to_sign, ingress_counter).encode()).unwrap();

        // Signature validation is done in Validate_unsigned which is called when the OCW sends transactions
        assert_err!(
            mock_send_challenge_transaction_from_ocw(challenge.clone(), ingress_counter, bad_signature, validator),
...
fn test_challenge_own_event_challenges() {
	eth_events_test_with_validators().execute_with(|| {
        ...
        let ingress_counter = EthereumEvents::populate_events_pending_challenge(&validator.account_id, 1);

        let challenge = create_challenge(EthereumEvents::get_event_id(0), ChallengeReason::IncorrectResult, validator.account_id);
        let signature = validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, &challenge, ingress_counter).encode()).unwrap();

        assert_noop!(
            EthereumEvents::challenge_event(RawOrigin::None.into(), challenge, ingress_counter, signature, validator),
...
fn test_challenge_valid_challenge_multiple() {
	eth_events_test_with_validators().execute_with(|| {
        let mut ingress_counter = EthereumEvents::populate_events_pending_challenge(&account_id_1(), 3);

        let validator = get_validator(1);
        let challenge1 = create_challenge(EthereumEvents::get_event_id(0), ChallengeReason::IncorrectResult, validator.account_id);
        let signature = validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, &challenge1, ingress_counter).encode()).unwrap();
        ...
        ingress_counter += 1;
        let challenge2 = create_challenge(EthereumEvents::get_event_id(1), ChallengeReason::IncorrectResult, validator.account_id);
        let signature = validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, &challenge2, ingress_counter).encode()).unwrap();
        assert_ok!(EthereumEvents::challenge_event(
            RawOrigin::None.into(),
            challenge2.clone(),
            ingress_counter,
...
fn test_challenge_valid_challenge_invalid_result() {
	eth_events_test_with_validators().execute_with(|| {
        ...
        assert!(System::events().iter().any(|a| a.event == mock::Event::pallet_ethereum_events(
                crate::Event::<TestRuntime>::EventChallenged(
...
fn test_challenge_valid_challenge_invalid_event_data() {
	eth_events_test_with_validators().execute_with(|| {
        ...
        let validator = get_validator(1);
        let challenge = create_challenge(EthereumEvents::get_event_id(0), ChallengeReason::IncorrectEventData, validator.account_id);
        let signature = validator.key.sign(&(CHALLENGE_EVENT_CONTEXT, &challenge, ingress_counter).encode()).unwrap();
