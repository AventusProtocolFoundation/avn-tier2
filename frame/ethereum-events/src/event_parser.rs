...
use simple_json2::json::{JsonValue, JsonObject};
use simple_json2::impls::SimpleError;
use simple_json2::parser::Error;
use hex::FromHex;
...
// TODO [TYPE: refactoring][PRI: unknown][NOTE: clarify]: extend the parser to work with named properties (what is this? Is it needed?)
...
pub fn find_event(events: &Vec<JsonValue>, topic: H256) -> Option<(&JsonValue, H160)> {
    let event = events.into_iter().find(|event| {topic_matches(event, topic).map_or_else(|_| false, |v| v)});
...
pub fn get_num_confirmations(json_response: &JsonValue) -> Result<u64, SimpleError> {
    let num_confirmations = get_value_of(String::from("num_confirmations"), json_response.get_object()?)?.get_number_f64()?;
...
fn hex_to_bytes(hex_string: String) -> Result<Vec<u8>, SimpleError> {
    ...
    return Vec::from_hex(hex_string).map_or_else(
        |_error| Err(SimpleError::plain_str("hex_to_bytes error")),
        |bytes| Ok(bytes)
    )
}
