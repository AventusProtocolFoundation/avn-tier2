...
fn create_db_entry_data(operation: OcwOperationExpiration) -> (u64, u64, PersistentId, LocalDBEntry::<u64,u64>) {
...
fn create_local_db_entry_with_new_slow_success() {
    let (starting_block, data_to_store, persistent_id, new_db_entry) = create_db_entry_data(OcwOperationExpiration::Slow);
...
fn create_local_db_entry_with_new_fast_success() {
    let (starting_block, data_to_store, persistent_id, new_db_entry) = create_db_entry_data(OcwOperationExpiration::Fast);
...
fn create_local_db_entry_with_new_custom_success() {
    ...
    let (starting_block, data_to_store, persistent_id, new_db_entry) = create_db_entry_data(
        OcwOperationExpiration::Custom(custom_block_expiry)
    );

    assert_eq!(
        new_db_entry.expiry,
        starting_block + u64::from(custom_block_expiry)
    );
    assert_eq!(new_db_entry.data, data_to_store);
    assert_eq!(new_db_entry.persistent_id, persistent_id);
}
...
fn unique_data_added_to_storage_and_expiry_list_success() {
    ...
    t.execute_with(|| {
        let (starting_block, _, persistent_id, db_entry) = create_db_entry_data(OcwOperationExpiration::Slow);
        assert!(set_lock_with_expiry(starting_block, OcwOperationExpiration::Slow, persistent_id).is_ok());
...
fn unique_data_added_twice_to_storage_fails() {
    ...
    t.execute_with(|| {
        let (starting_block, _, persistent_id, _) = create_db_entry_data(OcwOperationExpiration::Slow);

        assert!(set_lock_with_expiry(starting_block, OcwOperationExpiration::Slow, persistent_id.clone()).is_ok());
        assert!(set_lock_with_expiry(starting_block, OcwOperationExpiration::Slow, persistent_id.clone()).is_err());
...
fn expired_items_are_removed_successfully() {
    ...
    t.execute_with(|| {
        let (starting_block, _, first_id, db_entry) = create_db_entry_data(OcwOperationExpiration::Slow);
        let second_id = generate_db_entry_name(100);

        assert!(set_lock_with_expiry(starting_block, OcwOperationExpiration::Slow, first_id.clone()).is_ok());
        assert!(set_lock_with_expiry(starting_block, OcwOperationExpiration::Slow, second_id.clone()).is_ok());
...
fn expiry_removes_previous_blocks_as_well_successfully() {
    ...
    t.execute_with(|| {
        ...
        assert!(set_lock_with_expiry(earlier_block, OcwOperationExpiration::Slow, second_id.clone()).is_ok());
        assert!(set_lock_with_expiry(starting_block, OcwOperationExpiration::Slow, first_id.clone()).is_ok());
...
mod storage_locks {
    ...
    impl Context {
        fn insert_slow_lock(&self) {
            assert!(set_lock_with_expiry(self.start_block, self.operation.clone(), self.id.clone()).is_ok());
        }
        fn insert_other_slow_lock(&self) {
            assert!(set_lock_with_expiry(self.start_block, self.operation.clone(), self.other_id.clone()).is_ok());
        }
    ...
    mod delete_lock {
        ...
        mod removes_lock {
            ...
            fn when_lock_exists() {
                ...
                t.execute_with(|| {
                    ...
                    assert!(remove_storage_lock(context.start_block, context.operation, context.id.clone()).is_ok());
        ...
        mod fails_when {
            ...
            fn lock_does_not_exist() {
                ...
                t.execute_with(|| {
                    let context = Context::default();
                    assert!(remove_storage_lock(context.start_block, context.operation, context.id).is_err());
                });
            }
        }
    }
