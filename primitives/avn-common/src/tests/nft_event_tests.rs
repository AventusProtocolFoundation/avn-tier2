...
mod mint_nft {
    ...
    mod can_successfully_be_parsed {
        ...
        fn when_event_contains_correct_topics_and_data() {
            ...
            let expected_t2_owner_public_key = H256(hex!("1414141414141414141414141414141414141414141414141414141414141414"));
            let expected_sale_index = u64::from_be_bytes([1u8;8]);
            let expected_unique_external_ref: Vec<u8> = vec![8;36];

            let topics = vec![config.topic1, config.topic_batch_id, config.topic_sale_index, config.topic_owner_pk];
    ...
    mod fails_parsing_when {
        ...
        fn event_contains_too_many_topics() {
            ...
            let topics = vec![config.topic1.clone(), config.topic_batch_id, config.topic1, config.topic_sale_index.clone(), config.topic_sale_index];
            let result = NftMintData::parse_bytes(config.data, topics);
        ...
        fn event_contains_short_topic_batch_id() {
            ...
            let topics = vec![config.topic1, config.bad_topic_short, config.topic_sale_index, config.topic_owner_pk];
            let result = NftMintData::parse_bytes(config.data, topics);
        ...
        fn event_contains_short_topic_sale_index() {
            ...
            let topics = vec![config.topic1, config.topic_batch_id, config.bad_topic_short, config.topic_owner_pk];
            let result = NftMintData::parse_bytes(config.data, topics);
        ...
        fn event_contains_short_topic_owner() {
            ...
            let topics = vec![config.topic1, config.topic_batch_id, config.topic_sale_index, config.bad_topic_short];
            let result = NftMintData::parse_bytes(config.data, topics);
        ...
        fn event_contains_long_topic_batch_id() {
            ...
            let topics = vec![config.topic1, config.bad_topic_long, config.topic_sale_index, config.topic_owner_pk];
            let result = NftMintData::parse_bytes(config.data, topics);
        ...
        fn event_contains_long_topic_sale_index() {
            ...
            let topics = vec![config.topic1, config.topic_batch_id, config.bad_topic_long, config.topic_owner_pk];
            let result = NftMintData::parse_bytes(config.data, topics);
        ...
        fn event_contains_long_topic_owner() {
            ...
            let topics = vec![config.topic1, config.topic_batch_id, config.topic_sale_index, config.bad_topic_long];
            let result = NftMintData::parse_bytes(config.data, topics);
    ...
    mod result_is_invalid_when {
        ...
        fn event_contains_zero_address_for_owner() {
            ...
            let topics = vec![config.topic1, config.topic_batch_id, config.topic_sale_index, zero_address_owner];
            let result = NftMintData::parse_bytes(config.data, topics);
        ...
        fn event_contains_zero_batch_id() {
            ...
            let topics = vec![config.topic1, zero_batch_id, config.topic_sale_index, config.topic_owner_pk];
            let result = NftMintData::parse_bytes(config.data, topics);
...
mod transfer_to {
    ...
    mod can_successfully_be_parsed {
        ...
        fn when_event_contains_correct_topics_and_data() {
            ...
            let expected_t2_transfer_to_public_key = H256(hex!("1414141414141414141414141414141414141414141414141414141414141414"));
    ...
    mod fails_parsing_when {
        ...
        fn event_contains_too_many_topics() {
            ...
            let topics = vec![config.topic1.clone(), config.topic2, config.topic1, config.topic3.clone(), config.topic3];
            let result = NftTransferToData::parse_bytes(config.data, topics);
...
mod cancel_single_listing {
    ...
    mod fails_parsing_when {
        ...
        fn event_contains_too_many_topics() {
            ...
            let topics = vec![config.topic1, config.topic2_nft_id, config.topic3_op_id.clone(), config.topic3_op_id];
            let result = NftCancelListingData::parse_bytes(config.data, topics);
...
mod end_batch_listing {
    ...
    impl NftEndBatchListingConfig {
        fn setup() -> Self {
            ...
            NftEndBatchListingConfig {
                topic1: vec![1;32],
                topic2_batch_id: topic2_batch_id,
                data: None,
                bad_topic_short: vec![10;16],
                bad_topic_long: vec![10;64],
                bad_data: Some(vec![2;1]),
    ...
    mod fails_parsing_when {
        ...
        fn event_contains_too_many_topics() {
            ...
            let topics = vec![config.topic1, config.topic2_batch_id.clone(), config.topic2_batch_id];
            let result = NftEndBatchListingData::parse_bytes(config.data, topics);
